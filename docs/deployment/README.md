...
- **Plan** — [Product Deployment Foundation](product-deployment-foundation.md): Mac Control Station / Z2 PPU role boundaries, immutable release direction, and the first read-only product readiness audit.
...
The current `scripts/plasmactl` contract remains an integration-host contract. It must not be treated as the future Mac Control Station or Z2 product installer.
