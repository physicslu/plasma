...
The macOS installer pilot is the first host-mutating product adapter. It is deliberately unsigned and non-notarized, keeps Node.js and Python as external prerequisites, and must not be described as production Apple distribution readiness. The Z2 PS Phase-1 PPU work establishes runtime/release packaging and a manual, evidence-driven deployment boundary; production-grade PPU install/upgrade/rollback remains separate work.
