...
`plasma-web.service` is the existing compatibility-sensitive unit name; its product-facing role is **Plasma Gateway**.
...
The Plasma Gateway uses Python standard-library `ThreadingHTTPServer` and REST polling. It is not FastAPI and does not use WebSocket.
...
- add Engineering Mock Provider Plasma Gateway arguments only when `PLASMA_ENGINEERING_MOCK_ENABLED=1`;
...
The current schema is v6. Generic example:
...
`PLASMA_GATEWAY_HOST` and `PLASMA_GATEWAY_PORT` are retained deployment identifiers for the Plasma Gateway listener. They do not denote the Linux Default Gateway.

`PLASMA_PUBLIC_API_URL` is compatibility state and is empty in schema v6; current Browser routing is same-origin rather than Browser-owned remote Plasma Gateway Endpoint configuration.
...
- unknown/custom values are preserved as explicit operator overrides unless the schema explicitly retires their ownership;
- current schema completeness checks append only missing assignments and preserve existing operator values;
- schema v5 added `PLASMA_MANAGER_PPU_ALIAS` and never guessed a target alias;
- schema v6 retires Browser-owned direct Plasma Gateway Endpoints and clears `PLASMA_PUBLIC_API_URL` so same-origin routing becomes authoritative;
