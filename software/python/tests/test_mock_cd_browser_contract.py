...
CACHE_SPEC = ROOT / "software" / "web" / "e2e" / "tests" / "engineering-programming-image-cache-runtime.spec.ts"
...
def test_browser_workflow_is_mock_only_and_fail_closed() -> None:
    ...
    assert "engineering_1mib_programming_image_cache_reuse_and_reconnect" in source
...
def test_engineering_cache_spec_uses_real_gateway_without_route_mocking() -> None:
    ...
    assert "api/programming-images/check" in source
    assert "counters.uploads" in source
    assert "previous_session_id" in source
    assert 'Object.hasOwn(body, "image_base64")' in source
    assert 'Object.hasOwn(body, "firmware_sha256")' in source
...
def test_browser_playwright_configs_keep_real_stack_suite_isolated() -> None:
    ...
    assert "engineering-programming-image-cache-runtime.spec.ts" in source
    ...
    assert '"engineering-programming-image-cache-runtime.spec.ts"' in default_source
