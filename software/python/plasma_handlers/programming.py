...
class ProgrammingOperationHandler(BaseHandler):
    """Generic execution-stage dispatcher for a pre-admitted programming route.

    Target identity, Programming Profile selection and backend admission belong
    to the Site execution router. This handler only translates one admitted Job
    operation into the selected Site interface.
    """
    ...
    async def execute(self, request: JobRequest, stage_callback: StageCallback) -> ExecutionOutput:
        # SiteWorker invokes the handler once per attempt. Backend/profile
        # selection has already happened before the Job enters the queue.
        ...
        if operation is Operation.PROGRAM:
            ...
            if request.image_ref is not None:
                operation_call = lambda progress: self.interface.program_image_ref(
                    request.image_ref, 0, progress
                )
            else:
                operation_call = lambda progress: self.interface.program(request.image, 0, progress)
            await self._stage(
                "program",
                stage_callback,
                operation_call,
            ...
            return ExecutionOutput(
                details={"stages": ["program"], "bytes_programmed": request.image_size}
            )
        ...
        if operation is Operation.VERIFY:
            ...
            if request.image_ref is not None:
                operation_call = lambda progress: self.interface.verify_image_ref(
                    request.image_ref, 0, progress
                )
            else:
                operation_call = lambda progress: self.interface.verify(request.image, 0, progress)
            await self._stage(
                "verify",
                stage_callback,
                operation_call,
            ...
            return ExecutionOutput(
                details={"stages": ["verify"], "bytes_verified": request.image_size}
            )
        ...
        raise PlasmaError(
            ...
            f"operation '{operation.value}' is not handled by ProgrammingOperationHandler",
        )
