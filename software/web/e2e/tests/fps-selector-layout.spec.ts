...
test("desktop FPS selector uses the remaining viewport instead of a 52vh tree cap", async ({ page }) => {
  ...
  expect(geometry.selectorHeight).toBeGreaterThanOrEqual(760);
  ...
  expect(geometry.treeClientHeight).toBeGreaterThan(560);
...
test("iPad landscape keeps an operator-sized FPS column and collapses it back to a compact rail", async ({ page }) => {
  ...
  expect(expanded.selectorHeight).toBeGreaterThanOrEqual(690);
