...
export default function PpuRegistrationPage() {
  ...
  async function registerPpu() {
    ...
    await runMutation(
      ...
      `${selectedEntry.alias} validated and registered for managed programming.`,
    );
  }
  ...
  async function disableRegistration() {
    ...
    await runMutation(
      ...
      `${selectedEntry.alias} programming Registration disabled.`,
    );
  }
    ...
    <section className="ppuSiteConfiguration" aria-label="PPU Registration">
      <header className="ppuSiteHeader">
        <div>
          <small>PPU</small>
          <h2>Registration</h2>
          <p>Manage which known PPU connections are admitted to this Console for programming operations. Platform maintenance remains a separate lifecycle.</p>
        </div>
          ...
          <table className="ppuTable">
            ...
            <tbody>
              {(registry?.ppus ?? []).map(entry => {
                ...
                return (
                  <tr key={entry.alias ?? entry.endpoint} className={entry.alias === effectiveAlias ? "selected" : ""} onClick={() => { if (entry.alias) setSelectedAlias(entry.alias); }}>
                    <td><span className="ppuIdLink">{entry.alias ?? "Unaliased"}</span></td>
                    <td>{fleetView?.identity.ppu_id ?? "Awaiting probe"}</td>
                    <td><span className="ppuDimensionPill" data-tone={lifecycle.tone}>{entry.lifecycle === "commissioned" ? "Registered" : entry.lifecycle === "disabled" ? "Registered / Disabled" : "Not Registered"}</span></td>
                    <td><span className="ppuDimensionPill" data-tone={connectivity.tone}>{connectivity.label}</span></td>
                    <td><span className="ppuDimensionPill" data-tone={health.tone}>{health.label}</span></td>
      ...
      <p className="ppuRegistryBoundary">Registration controls managed programming admission. Bootstrap maintenance authorization used by Platform Firmware update is a separate Platform security boundary.</p>
    </section>
  );
}
