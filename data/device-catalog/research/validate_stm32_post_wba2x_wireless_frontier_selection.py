#!/usr/bin/env python3
"""Fail-closed validation for post-WBA2X STM32 wireless frontier reselection."""
...
SELECTION = HERE / "stm32-post-wba2x-wireless-frontier-selection.json"
...
def validate(value: dict) -> None:
    ...
    req(value.get("selection_id") == "stm32-post-wba2x-wireless-frontier-selection-v1", "selection id drifted")
    ...
    req(prod.get("exact_icpns") == 2523, "Production exact count drifted")
    req(prod.get("families") == 19, "Production family count drifted")
    req(prod.get("published_wireless_families") == {"STM32WBA2X": 14}, "published wireless boundary drifted")
    req(prod.get("standard_nonwireless_frontier_exhausted") is True, "non-wireless frontier reopened")
    req(prod.get("frozen_manifest_git_blob_sha") == "b2c6850e0c9246a8cf344ac7090b3f3d01573f1e", "Production blob drifted")
    ...
    req(upstream.get("published_wireless_count") == 1, "published wireless count drifted")
    req(upstream.get("remaining_structurally_eligible_count") == 4, "remaining eligible count drifted")
    ...
    req(isinstance(published, list) and len(published) == 1, "published wireless list drifted")
    ...
    req(value.get("selected_target_config") == "tcl/target/stm32wlx.cfg", "selected target config drifted")
    req(value.get("selected_row_count") == 17, "selected row count drifted")
    req(value.get("selected_subfamily_count") == 4, "selected subfamily count drifted")
    req(
        value.get("selected_subfamilies")
        == ["STM32WL54", "STM32WL55", "STM32WLE4", "STM32WLE5"],
        "selected subfamilies drifted",
    )
    req(value.get("selected_identifier_kind_counts") == {"ordering_pattern": 17}, "selected identifier mix drifted")
...
def main() -> int:
    ...
    validate(frozen)
    req(build_selection() == frozen, "post-WBA2X wireless selection is not deterministic replay")

    controls = [
        ("reselect-published-wba2x", lambda v: v.__setitem__("selected_wireless_frontier", "STM32WBA2X")),
        ("select-wba6x", lambda v: v.__setitem__("selected_wireless_frontier", "STM32WBA6X")),
        ("select-w108", lambda v: v.__setitem__("selected_wireless_frontier", "STM32W108")),
        ("wrong-target", lambda v: v.__setitem__("selected_target_config", "tcl/target/stm32wba6x.cfg")),
        ("drop-wba2x-publication", lambda v: v["production_boundary"].__setitem__("published_wireless_families", {})),
        ...
        ("open-wlx-admission", lambda v: v["claims"].__setitem__("stm32wlx_admission_ready", True)),
        ("skip-accessibility", lambda v: v.__setitem__("next_gate", "stm32wlx-bounded-exact-icpn-discovery-gate")),
    ]
    for name, mutate in controls:
        expect_rejected(name, mutate)

    print("STM32 post-WBA2X wireless frontier selection: PASS")
    print("published=1 eligible=4 deferred=1 selected=STM32WLX")
