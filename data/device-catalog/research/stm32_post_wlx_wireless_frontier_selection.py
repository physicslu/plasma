...
FROZEN_PRODUCTION = HERE / "stm32-post-wlx-production-prestate.json"
...
EXPECTED_PRODUCTION_EXACT = 2554
EXPECTED_PRODUCTION_FAMILIES = 20
PUBLISHED_WIRELESS = {"STM32WBA2X": 14, "STM32WLX": 31}
...
SELECTED = "STM32WBA6X"
NEXT_GATE = "stm32wba6x-bounded-official-manufacturer-evidence-accessibility-gate"
...
def _production_boundary() -> tuple[dict[str, Any], set[str]]:
    ...
    _require(
        ...
        "frozen post-WLX Production prestate drifted",
    )
...
def _remaining_candidates(published: set[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    _require(
        len(already_published) == 2
        and already_published[0]["plasma_series"] == "STM32WBA2X"
        and already_published[0]["production_row_count"] == 14
        and already_published[1]["plasma_series"] == "STM32WLX"
        and already_published[1]["production_row_count"] == 31,
...
def build_selection() -> dict[str, Any]:
    ...
    _require(selected["row_count"] == 19, "STM32WBA6X row count drifted")
    _require(selected["subfamily_count"] == 5, "STM32WBA6X subfamily count drifted")
    _require(
        selected["subfamilies"] == ["STM32WBA62", "STM32WBA63", "STM32WBA64", "STM32WBA65", "STM32WBA6M"],
        "STM32WBA6X subfamilies drifted",
    )
    _require(
        selected["target_configs"] == ["tcl/target/stm32wba6x.cfg"],
        "STM32WBA6X target config drifted",
    )
    _require(
        selected["identifier_kind_counts"] == {"ordering_pattern": 19},
        "STM32WBA6X identifier mix drifted",
    )

    return {
        "schema_version": 1,
        "selection_id": "stm32-post-wlx-wireless-frontier-selection-v1",
        ...
        "selection_basis": [
            "STM32WBA2X and STM32WLX are already published and are excluded from remaining frontier selection",
            "remaining wireless STM32 candidates stay under the dedicated wireless scope boundary",
            "STM32WBA6X is the smallest remaining structurally eligible bounded wireless candidate by the frozen sequencing policy",
