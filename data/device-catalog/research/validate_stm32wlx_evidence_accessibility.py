#!/usr/bin/env python3
"""Fail-closed validator for retained STM32WLX official-ST accessibility evidence."""
...
EVIDENCE_DIR = HERE / "evidence" / "stm32wlx-accessibility-live-2026-09-21"
...
EXPECTED_NEXT_GATE = "stm32wlx-bounded-exact-icpn-discovery-gate"
...
def validate_documents(
    ...
    req(targets == expected_manifest, "retained STM32WLX target manifest is not deterministic replay")
    req(targets.get("probe_id") == PROBE_ID, "target manifest probe id drifted")
    req(targets.get("target_count") == EXPECTED_TARGETS, "STM32WLX target count drifted")
    req(targets.get("candidate_source_sha256") == EXPECTED_SOURCE_SHA256, "candidate source digest drifted")

    anomalies = targets.get("known_non_prefix_ordering_rows")
    req(isinstance(anomalies, list) and len(anomalies) == 1, "WL5M anomaly record drifted")
    anomaly = anomalies[0]
    req(anomaly.get("subfamily") == "STM32WL55", "WL5M anomaly subfamily drifted")
    req(anomaly.get("part_number") == EXPECTED_NON_PREFIX_PATTERN, "WL5M anomaly pattern drifted")
    req(anomaly.get("derived_base_device") == EXPECTED_NON_PREFIX_BASE, "WL5M anomaly Base Device drifted")
    req(anomaly.get("representative_eligible") is False, "WL5M anomaly became representative-eligible")
    ...
    req(
        ...
        == EXPECTED_SUBFAMILIES,
        "STM32WLX deterministic subfamily order drifted",
    )
    req(
        tuple(item.get("base_device") for item in manifest_targets if isinstance(item, dict))
        == EXPECTED_BASES,
        "STM32WLX deterministic representative Base Devices drifted",
    )
    for item in manifest_targets:
        ...
        req(isinstance(base, str) and isinstance(subfamily, str) and base.startswith(subfamily), "representative escaped prefix-consistent subfamily boundary")
        req(
            isinstance(url, str)
            and url == f"https://www.st.com/en/microcontrollers-microprocessors/{base.lower()}.html",
    ...
    req(summary.get("successful_targets") == EXPECTED_TARGETS, "not all STM32WLX representative targets succeeded")
    req(summary.get("manual_review_targets") == 0, "manual review remains in STM32WLX accessibility probe")
    ...
    req(
        ...
        "official ST evidence is not accessible for all STM32WLX subfamilies",
    )
    ...
    req(summary.get("active_exact_icpns_observed_on_representative_pages") == EXPECTED_ACTIVE_OBSERVED, "representative Active exact-ICPN observation count drifted")
    req(summary.get("excluded_non_active_part_numbers_observed") == EXPECTED_EXCLUDED_OBSERVED, "representative lifecycle exclusion count drifted")
    ...
    for expected_subfamily, item in zip(EXPECTED_SUBFAMILIES, results, strict=True):
        ...
        base = item.get("base_device")
        req(isinstance(base, str) and base in EXPECTED_ACTIVE_COUNTS, f"{expected_subfamily}: unexpected representative Base Device")
        observed_bases.append(base)
        active_count = item.get("active_exact_icpn_count")
        excluded_count = item.get("excluded_non_active_count")
        req(active_count == EXPECTED_ACTIVE_COUNTS[base], f"{base}: representative Active count drifted")
        req(excluded_count == 0, f"{base}: representative exclusion count drifted")
        counted_active += int(active_count)
        counted_excluded += int(excluded_count)

        evidence = item.get("evidence")
        req(isinstance(evidence, dict), f"{base}: evidence missing")
        req(evidence.get("acquisition_transport") == BROWSER_TRANSPORT, f"{base}: evidence transport drifted")
        req(evidence.get("base_device") == base, f"{base}: evidence Base Device mismatch")
        req(evidence.get("source_url") == item.get("source_url"), f"{base}: evidence source URL mismatch")
        ...
        req(isinstance(exact, list) and len(exact) == EXPECTED_ACTIVE_COUNTS[base], f"{base}: retained Active identity count drifted")
        req(isinstance(excluded, list) and excluded == [], f"{base}: retained lifecycle exclusions drifted")
        req(all(isinstance(value, str) and value.startswith(base) for value in exact), f"{base}: foreign Active ICPN")
        req(
        ...
        req(retained == evidence, f"{base}: standalone evidence JSON differs from summary")
    ...
    req(provenance.get("browser_version") == EXPECTED_BROWSER_VERSION, "browser version drifted")
...
def main() -> None:
    ...
    controls = [
        ...
        ("open WLX admission", lambda t,s,p: s["claims"].__setitem__("stm32wlx_admission_ready", True)),
        ("normalize WL5M alias", lambda t,s,p: s["claims"].__setitem__("wl5m_alias_normalized", True)),
        ("make WL5M representative eligible", lambda t,s,p: t["known_non_prefix_ordering_rows"][0].__setitem__("representative_eligible", True)),
        ("skip exact discovery gate", lambda t,s,p: s.__setitem__("next_gate", "stm32wlx-production-publication-gate")),
        ("missing representative success", lambda t,s,p: s.__setitem__("successful_targets", 3)),
        ("wrong Active observation count", lambda t,s,p: s.__setitem__("active_exact_icpns_observed_on_representative_pages", 7)),
        ("wrong browser version", lambda t,s,p: p.__setitem__("browser_version", "0.0.0.0")),
    ...
    print("STM32WLX bounded official-ST evidence accessibility validation: PASS")
    print(
