#!/usr/bin/env python3
"""STM32C0 C0.3 deterministic manufacturer-authoritative metadata policy planner."""
...
from validate_stm32c0_phase_c0_2_retained_evidence import (
    BASELINE as DISCOVERY_BASELINE,
    DISCOVERY as DISCOVERY_MANIFEST,
...
ADMISSION_PHASE = "C0.4"
ADAPTER_ID = "stm32c0-c0.3-metadata"
...
class STM32C0PolicyError(AdmissionError):
    pass
...
def _string_set_sha256(values: set[str] | list[str]) -> str:
...
def metadata_contract() -> dict[str, Any]:
    return {
        ...
        "bounded_active_base_device_count": EXPECTED_TARGET_COUNT,
        "identity_lifecycle_authority": "retained C0.2 official ST Quality & Reliability exact identity + Sample & Buy Marketing Status exact-set join",
...
def _policy_evidence() -> dict[str, Any]:
    ...
    return {
        ...
        "semantic_decisions": {
            "c071_f_p": "F/P resolves to TSSOP20",
            "c071_f_y": "F/Y resolves to WLCSP19; pin count is package-dependent",
            "c071_option_N": "N is an official product-version option and is preserved",
            "c071_option_TR": "TR is packing and is distinct from N product version",
            "c071_option_NTR": "NTR is N product version plus tape-and-reel packing",
            "c091_c092_shared_authority": "DS14720 is shared but 091 and 092 remain distinct manufacturer series",
            "openocd_role": "routing observation only; not metadata authority or metadata gate",
...
def build_policy_plan(*, production_manifest_path: Path = DEFAULT_PRODUCTION_MANIFEST) -> dict[str, Any]:
    if validate_retained() != 0:
        raise STM32C0PolicyError("C0.2 retained evidence validator failed")
    provenance = read_json(EVIDENCE_DIR / "provenance.json")
    summary = read_json(C0_2_SUMMARY)
    discovery_baseline = read_json(DISCOVERY_BASELINE)
    if summary.get("discovery_id") != EXPECTED_EVIDENCE_ID:
        raise STM32C0PolicyError("C0.2 retained evidence identity drifted")
    if (
        ...
        or provenance.get("target_count") != EXPECTED_TARGET_COUNT
        or provenance.get("production_write_authorized") is not False
        or provenance.get("canonical_admission_authorized") is not False
    ):
        raise STM32C0PolicyError("C0.2 clean commercial boundary drifted")
    if discovery_baseline.get("claims", {}).get("production_write_authorized") is not False:
        raise STM32C0PolicyError("C0.2 cannot authorize Production")
    ...
    return {
        ...
        "base_device_count": len({str(item["base_device"]) for item in candidates}),
        "decision_counts": {
        ...
        "inputs": {
            ...
            "retained_discovery_manifest": DISCOVERY_MANIFEST.name,
            "retained_discovery_manifest_sha256": file_sha256(DISCOVERY_MANIFEST),
            "retained_summary": str(C0_2_SUMMARY.relative_to(HERE)),
            "retained_summary_sha256": file_sha256(C0_2_SUMMARY),
            "ordering_authority_manifest": DEFAULT_ORDERING_AUTHORITY.name,
            "ordering_authority_manifest_sha256": file_sha256(DEFAULT_ORDERING_AUTHORITY),
            "production_manifest": production_manifest_path.name,
            "production_manifest_git_blob_sha": _git_blob_sha(production_manifest_path),
...
def policy_plan_is_clean(plan: dict[str, Any]) -> bool:
    distribution = plan.get("metadata_distribution")
    option_distribution = distribution.get("option_suffix", {}) if isinstance(distribution, dict) else {}
    return (
        ...
        and plan.get("base_device_count") == EXPECTED_TARGET_COUNT
        and plan.get("decision_counts") == {"metadata_ready": 220, "manual_review_required": 0, "reject": 0}
        and plan.get("issues") == []
        and isinstance(distribution, dict)
        and set(option_distribution).issubset({"", "TR", "N", "NTR"})
        and option_distribution.get("N", 0) > 0
        and option_distribution.get("NTR", 0) > 0
...
def policy_summary(plan: dict[str, Any]) -> dict[str, Any]:
    rows = [item["metadata"] for item in plan["candidates"] if item["metadata"] is not None]
    exact_icpns = {str(item["icpn"]) for item in plan["candidates"]}
    base_devices = {str(item["base_device"]) for item in plan["candidates"]}
    return {
        "schema_version": 1,
        "phase": PHASE,
        "family": FAMILY,
        ...
        "base_device_count": plan["base_device_count"],
        "base_devices": sorted(base_devices),
        "base_device_set_sha256": _string_set_sha256(base_devices),
        "policy_ready_exact_icpn_set_sha256": _string_set_sha256(exact_icpns),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not policy_plan_is_clean(plan):
        ...
        raise STM32C0PolicyError("C0.3 metadata policy plan is not clean")
    ...
    diagnostic = {
        ...
        "policy_ready_exact_icpn_set_sha256": summary["policy_ready_exact_icpn_set_sha256"],
        "metadata_rows_sha256": summary["metadata_rows_sha256"],
    ...
    if args.write_baseline:
        ...
        raise STM32C0PolicyError("C0.3 policy baseline is not frozen yet")
    elif summary != read_json(args.baseline):
        raise STM32C0PolicyError("C0.3 policy baseline drifted")
    if args.output:
