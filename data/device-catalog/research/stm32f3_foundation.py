#!/usr/bin/env python3
"""Fail-closed STM32F3 source-surface foundation for bounded discovery.

This module deliberately stops before manufacturer evidence acquisition. It
qualifies the current OpenOCD-derived STM32F3 surface, extracts concrete Base
Devices from ordering patterns, selects one deterministic initial target per
source subfamily, and resolves synthetic routing examples to stm32f3x.cfg.
...
BASE_RE = re.compile(r"^STM32F3[0-9]{2}[A-Z][A-Z0-9]$")
PATTERN_RE = re.compile(r"^(STM32F3[0-9]{2}[A-Z][A-Z0-9])([A-Z])x$")
ROUTING_VALUE_RE = re.compile(r"^STM32F3[0-9A-Z]+$")
...
def guarded_rows(catalog_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    if len(rows) != EXPECTED_ROW_COUNT:
        raise AcquisitionError(
            f"{PHASE} requires the guarded {EXPECTED_ROW_COUNT}-row STM32F3 surface, got {len(rows)}"
        )

    subfamilies = Counter(row.get("subfamily", "") for row in rows)
    if dict(sorted(subfamilies.items())) != EXPECTED_SUBFAMILY_COUNTS:
        raise AcquisitionError(
            f"{PHASE} STM32F3 subfamily surface drifted: {dict(sorted(subfamilies.items()))}"
        )
    ...
    for row in rows:
        ...
        if match is None:
            raise AcquisitionError(f"unsupported STM32F3 ordering pattern: {pattern!r}")
        if BASE_RE.fullmatch(match.group(1)) is None:
            raise AcquisitionError(f"invalid concrete STM32F3 Base Device: {match.group(1)!r}")
...
def base_from_row(row: dict[str, str]) -> str:
    ...
    if match is None:
        raise AcquisitionError(f"unsupported STM32F3 ordering pattern: {row.get('part_number')!r}")
