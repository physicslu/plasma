#!/usr/bin/env python3
"""Run the authoritative STM32L1 official-ST lifecycle requalification probe."""
...
from stm32l1_requalification_probe import (
    DEFAULT_OUTPUT,
    PARSER_PROFILE,
    RateLimitedFetcher,
...
def main(argv: list[str] | None = None) -> int:
    ...
    with STDualSurfaceBrowserAcquirer(
        ...
        family_label="STM32L1 lifecycle requalification",
        headless=args.headless,
    ) as acquirer:
        fetcher = RateLimitedFetcher(delay_seconds=args.delay, fetcher=acquirer.fetch)
        summary = run_probe(targets=targets, fetcher=fetcher, timeout_seconds=args.timeout)
        summary["browser"] = {
            "headless": args.headless,
