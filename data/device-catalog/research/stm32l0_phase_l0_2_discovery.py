#!/usr/bin/env python3
"""Fail-closed STM32L0 L0.2 manufacturer-authoritative commercial discovery.

L0.2 expands the frozen L0.1 ordering-pattern surface into the complete
deterministic Base Device set, then observes exact commercial ICPNs and
lifecycle only from official ST evidence. Quality & Reliability is exact
identity authority; Sample & Buy is Marketing Status authority; exact Part
Number sets must join exactly before lifecycle disposition is accepted.

OpenOCD bounds research and supplies routing diagnostics only. This module does
not authorize canonical admission, Production writes, programming policy,
Flash/security qualification, HIL, electrical, or runtime support.
"""
...
from stm32l0_phase_l0_1_foundation import (
    DEFAULT_BASELINE as L0_1_BASELINE,
    DEFAULT_CATALOG,
...
DISCOVERY_ID = "stm32l0-l0.2-official-st-commercial-discovery-v1"
EXPECTED_L0_1_BASELINE_SHA256 = "958572fa69a4457966256851a39badfdc38490e0c3ce155978155baf3ca9e488"
EXPECTED_L0_1_REPRESENTATIVES = {base for _subfamily, base in EXPECTED_TARGETS}
...
DEFAULT_OUTPUT = Path("/tmp/stm32l0-l0.2-live-summary.json")
MAX_TARGETS = 164
...
def validate_l0_1_boundary(path: Path = L0_1_BASELINE) -> dict[str, Any]:
    if sha256(path) != EXPECTED_L0_1_BASELINE_SHA256:
        raise AcquisitionError("L0.1 foundation baseline SHA-256 drifted")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict) or payload.get("phase") != "L0.1" or payload.get("family") != FAMILY:
        raise AcquisitionError("unexpected L0.1 foundation baseline identity")
    if payload.get("source_row_count") != 166 or payload.get("target_config") != TARGET_CONFIG:
        raise AcquisitionError("L0.1 bounded research surface drifted")
    if payload.get("identifier_kind_counts") != {"cmsis_device_name": 2, "ordering_pattern": 164}:
        raise AcquisitionError("L0.1 identifier-kind boundary drifted")
    _all_false(payload.get("claims"), "L0.1 claims")
    reps = payload.get("initial_targets")
    observed = {item.get("base_device") for item in reps or [] if isinstance(item, dict)}
    if observed != EXPECTED_L0_1_REPRESENTATIVES:
        raise AcquisitionError("L0.1 representative continuity boundary drifted")
    return payload
...
def deterministic_targets(catalog_rows: list[dict[str, str]]) -> list[DiscoveryTarget]:
        ...
        for base in bases:
            targets.append(
                DiscoveryTarget(
                    ...
                    selection_reason=(
                        "deterministic unique Base Device collapsed from frozen L0.1 "
    ...
    if not bases or len(bases) > MAX_TARGETS or len(set(bases)) != len(bases):
        raise AcquisitionError(f"L0.2 invalid deterministic Base Device set: {len(bases)}")
    if not EXPECTED_L0_1_REPRESENTATIVES.issubset(set(bases)):
        raise AcquisitionError("L0.2 deterministic set lost a frozen L0.1 representative")
    return targets


def validate_targets(targets: list[DiscoveryTarget]) -> None:
    if not targets or len(targets) > MAX_TARGETS:
        raise AcquisitionError("L0.2 target count escaped frozen ordering-pattern bound")
    seen: set[str] = set()
    covered: set[str] = set()
    for target in targets:
        if target.subfamily not in EXPECTED_SUBFAMILIES:
            raise AcquisitionError(f"unsupported L0 subfamily: {target.subfamily}")
        if target.base_device in seen or not target.base_device.startswith(target.subfamily):
            raise AcquisitionError(f"invalid/duplicate L0 Base Device: {target.base_device}")
    ...
    if covered != set(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("L0.2 target manifest lost a frozen subfamily")
    if not EXPECTED_L0_1_REPRESENTATIVES.issubset(seen):
        raise AcquisitionError("L0.2 target manifest lost L0.1 representatives")
...
def target_manifest(targets: list[DiscoveryTarget]) -> dict[str, object]:
    ...
    return {
        ...
        "scope": "bounded read-only official-ST STM32L0 complete commercial discovery",
        "l0_1_foundation_sha256": EXPECTED_L0_1_BASELINE_SHA256,
...
def build_discovery_evidence_record(**kwargs: Any) -> dict[str, object]:
    ...
    if evidence.get("evidence_surface") != AUTHORITY_SURFACE:
        raise AcquisitionError("STM32L0 L0.2 evidence authority surface drifted")
    return evidence
    ...
    *,
    ...
    for result in results:
        ...
        if isinstance(evidence, dict):
            exact_set.update(icpn for icpn in evidence.get("exact_icpns", []) if isinstance(icpn, str))

    representative_results = {
        str(result["base_device"]): result
        for result in results
        if result.get("base_device") in EXPECTED_L0_1_REPRESENTATIVES
    }
    representative_continuity = (
        set(representative_results) == EXPECTED_L0_1_REPRESENTATIVES
    ...
    return {
        ...
        "family": FAMILY,
        "discovery_id": DISCOVERY_ID,
        "scope": "bounded read-only official-ST STM32L0 commercial identity/lifecycle discovery",
        "l0_1_foundation_sha256": EXPECTED_L0_1_BASELINE_SHA256,
        ...
        "openocd_routing": {
            "unique": routing["unique"], "ambiguous": routing["ambiguous"],
            "unmapped": routing["unmapped"], "not_applicable": routing["not_applicable"],
...
def write_evidence_files(summary: dict[str, object], evidence_dir: Path) -> None:
    ...
    for result in summary.get("results", []):
        ...
        if not isinstance(base, str) or not isinstance(evidence, dict):
            raise AcquisitionError("successful L0.2 result lacks Base Device/evidence")
