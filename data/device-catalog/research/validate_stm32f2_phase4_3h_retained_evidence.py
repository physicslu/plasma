#!/usr/bin/env python3
"""Validate immutable Phase 4.3H STM32F2 official-ST discovery evidence."""
...
DEFAULT_BASELINE = HERE / "stm32f2-phase4.3h-discovery-baseline.json"
DEFAULT_EVIDENCE_DIR = (
    HERE / "evidence" / "stm32f2-phase4.3h-official-st-discovery-live-2026-09-07"
)
...
def validate(*, evidence_dir: Path, baseline_path: Path) -> dict[str, Any]:
    ...
    require(baseline.get("phase") == "4.3H", "baseline phase mismatch")
    ...
    require(
        context
        == {
            "execution_mode": "local_headed_chromium",
            "workflow_run_id": None,
            "artifact_id": None,
            "artifact_zip_sha256": None,
        },
        "Phase 4.3H acquisition context drifted",
    )

    catalog = read_catalog(DEFAULT_CATALOG)
    production_bases = read_production_bases(DEFAULT_CANONICAL)
    pilot_id, manifest_targets = read_manifest(DEFAULT_MANIFEST, catalog, production_bases)
    ...
    require(summary.get("phase") == "4.3H", "retained summary phase mismatch")
    ...
    require(discovery_is_clean(summary), "retained Phase 4.3H discovery is not clean")
    ...
    require(core["exact_icpn_candidate_count"] == 11, "provenance candidate count mismatch")
    require(provenance.get("execution_mode") == "local_headed_chromium", "execution mode mismatch")
    require(provenance.get("workflow_run_id") is None, "Phase 4.3H must not claim a workflow run")
    require(provenance.get("artifact_id") is None, "Phase 4.3H must not claim a workflow artifact")
    require(provenance.get("artifact_zip_sha256") is None, "Phase 4.3H must not claim an artifact digest")
    require(provenance.get("excluded_non_active_part_number_count") == 0, "lifecycle exclusion count mismatch")
    require(provenance.get("production_admission_ready") is False, "retained evidence cannot authorize admission")
    require(provenance.get("playwright_version") == runtime.get("playwright_requirement"), "Playwright provenance mismatch")
    require(provenance.get("chromium_version") == runtime.get("browser_version"), "Chromium provenance mismatch")
    ...
    require(mapping_count == 11, "retained exact ICPN mapping count mismatch")
    ...
    return {
        ...
        "active_exact_icpn_candidates": 11,
        "excluded_non_active_part_numbers": 0,
        "unique_openocd_mappings": 11,
