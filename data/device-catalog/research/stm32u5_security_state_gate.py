...
from stm32u5_admission_policy import build_plan as build_canonical_plan, plan_is_clean
from stm32u5_metadata_policy import FAMILY, load_security_fence

HERE = Path(__file__).resolve().parent
MODEL = HERE / "stm32u5-security-state-model.json"
TRANSACTION = "stm32u5-security-state-admission-gate"
...
EXPECTED_LIFECYCLE_TRANSITION_IDS = {
    "enable_trustzone",
    "raise_to_rdp1",
    "rdp1_to_lower_protection_unresolved",
    "enter_rdp2",
...
def _read_json(path: Path = MODEL) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise AdmissionError("STM32U5 security-state model root must be object")
    return value
...
def validate_model(model: dict[str, Any]) -> None:
    ...
    _require(model.get("canonical_active_identity_count") == 265, "canonical Active identity count drifted")
    _require(model.get("quarantined_preview_exact_icpns") == ["STM32U5G9ZJJ3Q"], "Preview quarantine drifted")
    _require(model.get("production_exact_icpn_count") == 2017, "Production exact ICPN count drifted")
    ...
    for source in sources:
        ...
        _require(urlparse(url).hostname in {"www.st.com", "st.com", "wiki.st.com"}, f"{sid}: non-ST source host")
        observations = source.get("observations")
        _require(isinstance(observations, list) and observations, f"{sid}: observations missing")
        source_ids.add(sid)
    _require({"st-rm0456", "st-wiki-rdp"}.issubset(source_ids), "required STM32U5 security authorities missing")

    oem_policy = model.get("oem_state_policy")
    _require(isinstance(oem_policy, dict), "OEM state policy missing")
    for key in (
        "orthogonal_to_lifecycle_state", "key_material_must_never_be_stored", "key_material_must_never_be_logged",
        "lock_and_provisioning_state_must_be_observed_before_regression_decision", "unknown_oem_state_is_deny",
    ):
        _require(oem_policy.get(key) is True, f"OEM policy weakened: {key}")
    for key in ("self_attested_oem_state_is_sufficient", "oem_unlock_execution_authorized", "oem_key_provisioning_authorized"):
    ...
    for state in states:
        ...
        _require(state.get("unconditionally_terminal") is False, f"{sid}: lifecycle state must not be modeled unconditionally terminal")
        if rdp == "2":
    ...
    unresolved_rdp1 = by_transition["rdp1_to_lower_protection_unresolved"]
    _require(set(unresolved_rdp1["from"]) == {"TZ0_RDP1", "TZ1_RDP1"}, "RDP1 regression source boundary drifted")
    _require(set(unresolved_rdp1["to"]) == {"TZ0_RDP0", "TZ1_RDP0", "TZ1_RDP0_5"}, "RDP1 regression target boundary drifted")
    _require(unresolved_rdp1.get("semantics_status") == "unresolved_fail_closed", "RDP1 regression semantics were falsely resolved")
    rdp1_requires = unresolved_rdp1.get("requires")
    _require(isinstance(rdp1_requires, list) and any("OEM1/OEM2" in item for item in rdp1_requires), "RDP1 regression lost OEM-state prerequisite")

    enter2 = by_transition["enter_rdp2"]
    _require(enter2.get("unconditionally_irreversible") is False, "STM32U5 RDP2 must not be modeled unconditionally irreversible")
    _require(enter2.get("conditional_regression_requires_oem2") is True, "STM32U5 RDP2 OEM2 dependency missing")
    ...
    _require(isinstance(unresolved, list) and len(unresolved) >= 4, "unresolved security semantics boundary missing")
    _require(any("RDP1 regression" in item for item in unresolved), "RDP1 regression uncertainty was hidden")
    ...
    _require(operations.get("read_security_state") == "blocked_pending_runtime_observer_contract", "security observer gate drifted")
    ...
    for key in (
        "security_state_admission_model_complete", "destructive_transition_policy_fail_closed",
        "rdp2_conditional_regression_boundary_modeled", "oem_transition_dependency_modeled",
        "unknown_oem_state_fail_closed", "execution_state_dependency_modeled",
        "exact_rdp1_regression_semantics_left_unresolved", "canonical_identity_plan_remains_valid",
        "preview_quarantine_remains_enforced", "hil_required_before_runtime_enablement",
    ):
        _require(admission.get(key) is True, f"{key}: required gate result missing")
    for key in (
        "all_security_transition_semantics_validated", "rdp2_unconditionally_terminal",
        "production_manifest_admission_authorized", "runtime_programming_authorized",
        "security_mutation_authorized", "oem_key_operation_authorized",
    ):
        _require(admission.get(key) is False, f"{key}: unsafe/overstated gate result")
    _require(model.get("next_research_gate") == "stm32u5-runtime-enforcement-admission-gate", "next gate drifted")


def validate_upstream() -> None:
    foundation = load_security_fence()
    partition = foundation.get("research_partition", {})
    _require(partition.get("catalog_admission_governed_separately") is True, "catalog-admission separation drifted")
    for key in (
    ...
    _require(invariants.get("catalog_admission_independent_from_hil") is True, "catalog/HIL separation drifted")
    _require(invariants.get("catalog_membership_does_not_authorize_execution") is True, "catalog execution boundary weakened")

    plan = build_canonical_plan()
    _require(plan_is_clean(plan), "upstream STM32U5 canonical plan is not clean")
    _require(plan.get("retained_exact_icpns") == 266, "upstream retained identity count drifted")
    _require(plan.get("planned_canonical_rows") == 265, "upstream canonical row count drifted")
    _require(plan.get("quarantined_preview_exact_icpns") == ["STM32U5G9ZJJ3Q"], "upstream Preview quarantine drifted")
    _require(plan.get("production_exact_icpn_count") == 2017, "upstream Production exact ICPN count drifted")
    ...
    _require(plan.get("claims", {}).get("preview_identity_admission_authorized") is False, "upstream canonical plan admitted Preview identity")
...
def build_gate_result(model_path: Path = MODEL) -> dict[str, Any]:
    ...
    return {
        ...
        "quarantined_preview_identity_count": 1,
        ...
        "exact_rdp1_regression_semantics_left_unresolved": True,
...
def negative_control_mutations(model: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    bad_rdp05["states"].append({
        ...
        "execution_state": "not_applicable", "debug_access": "open", "unconditionally_terminal": False,
    })
    cases.append(("RDP0.5 without TrustZone", bad_rdp05))

    resolved_rdp1 = copy.deepcopy(model)
    next(item for item in resolved_rdp1["lifecycle_transition_rules"] if item["id"] == "rdp1_to_lower_protection_unresolved")["semantics_status"] = "validated"
    cases.append(("RDP1 regression semantics falsely resolved", resolved_rdp1))
