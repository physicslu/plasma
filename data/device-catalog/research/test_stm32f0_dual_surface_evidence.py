...
def page(*, lifecycle_rows: str, quality_extra: str = "") -> str:
    ...
    <html><body>
      ...
      <table>
        ...
        <tr><td>STM32F030C6T6</td><td>Ecopack2</td><td>Industrial</td></tr>
        <tr><td>STM32F030C6T6TR</td><td>Ecopack2</td><td>Industrial</td></tr>
      </table>
      {quality_extra}
      <h2>Sample &amp; Buy</h2>
      <table>
...
ACTIVE_ROWS = """
<tr><td>STM32F030C6T6</td><td>Active Product is in volume production.</td><td>LQFP48</td></tr>
<tr><td>STM32F030C6T6TR</td><td>Active Product is in volume production.</td><td>LQFP48</td></tr>
"""


class STM32F0DualSurfaceEvidenceTests(unittest.TestCase):
    ...
    def test_evidence_record_uses_stm32f0_owned_profile(self) -> None:
        ...
        self.assertEqual(PARSER_PROFILE, "stm32f0_dual_surface_v1")
        self.assertEqual(evidence["exact_icpns"], ["STM32F030C6T6", "STM32F030C6T6TR"])
    ...
    def test_non_active_identity_is_retained_but_excluded_from_active_set(self) -> None:
        rows = ACTIVE_ROWS.replace(
            "<tr><td>STM32F030C6T6TR</td><td>Active Product is in volume production.</td>",
            "<tr><td>STM32F030C6T6TR</td><td>NRND Not Recommended for New Designs.</td>",
        )
        evidence = build_dual_surface_browser_evidence_record(
        ...
        self.assertEqual(evidence["exact_icpns"], ["STM32F030C6T6"])
        self.assertEqual(
            evidence["excluded_non_active_part_numbers"],
            [{
                "icpn": "STM32F030C6T6TR",
    ...
    def test_extra_foreign_lifecycle_identity_fails_closed(self) -> None:
        rows = ACTIVE_ROWS + (
            "<tr><td>STM32F031C4T6</td>"
    ...
    def test_foreign_quality_identity_fails_closed(self) -> None:
        html = page(
            ...
            quality_extra="<div>STM32F031C4T6</div>",
        )
        with self.assertRaisesRegex(AcquisitionError, "foreign STM32"):
            extract_dual_surface_part_number_records(html, BASE)

    def test_duplicate_lifecycle_identity_fails_closed(self) -> None:
        rows = ACTIVE_ROWS + ACTIVE_ROWS.splitlines()[1] + "\n"
        with self.assertRaisesRegex(AcquisitionError, "duplicate exact ICPN"):
            extract_dual_surface_part_number_records(page(lifecycle_rows=rows), BASE)

    def test_conflicting_lifecycle_identity_fails_closed(self) -> None:
        rows = ACTIVE_ROWS + (
            "<tr><td>STM32F030C6T6</td><td>NRND Not Recommended for New Designs.</td>"
            "<td>LQFP48</td></tr>"
        )
        with self.assertRaisesRegex(AcquisitionError, "conflicting"):
            extract_dual_surface_part_number_records(page(lifecycle_rows=rows), BASE)
