#!/usr/bin/env python3
"""Validate checked-in STM32WBX Production catalog publication bytes."""
...
from publish_stm32wbx_catalog import (
    CANONICAL,
...
PROPOSAL = HERE / "stm32wbx-production-publication-proposal.json"
...
def main() -> int:
    ...
    req(CANONICAL.read_bytes() == canonical_bytes, "checked-in STM32WBX canonical CSV differs from frozen source")
    req(PRODUCTION_MANIFEST.read_bytes() == manifest_bytes, "checked-in Production manifest differs from deterministic STM32WBX poststate")
    expected_proposal = (json.dumps(proposal, indent=2, sort_keys=True) + "\n").encode("utf-8")
    req(PROPOSAL.read_bytes() == expected_proposal, "checked-in STM32WBX publication proposal differs from deterministic renderer")

    rows = list(csv.DictReader(io.StringIO(canonical_bytes.decode("utf-8"))))
    req(tuple(rows[0].keys()) == PRODUCTION_FIELDS if rows else False, "STM32WBX canonical schema drifted")
    req(len(rows) == EXPECTED_ROWS, f"STM32WBX canonical row count drifted: {len(rows)}")
    icpns = {row["icpn"] for row in rows}
    bases = {row["base_device"] for row in rows}
    req(len(icpns) == EXPECTED_ROWS, "STM32WBX canonical ICPNs are not unique")
    req(set_sha(icpns) == EXPECTED_EXACT_SET_SHA256, "STM32WBX exact ICPN set digest drifted")
    req(len(bases) == EXPECTED_ACTIVE_BASES, "STM32WBX Active Base Device count drifted")
    req({row["family"] for row in rows} == {FAMILY}, "STM32WBX canonical file contains foreign family")
    req({row["manufacturer"] for row in rows} == {MANUFACTURER}, "STM32WBX manufacturer drifted")
    req(Counter(row["series"] for row in rows) == Counter(EXPECTED_SERIES_COUNTS), "STM32WBX series counts drifted")
    req(Counter(row["existing_identifier_kind"] for row in rows) == Counter(EXPECTED_ROUTE_KIND_COUNTS), "STM32WBX route assignment counts drifted")
    req(Counter(row["mapping_status"] for row in rows) == Counter(EXPECTED_MAPPING_COUNTS), "STM32WBX mapping counts drifted")
    req(all(row["openocd_target_config"] == TARGET_CONFIG for row in rows), "STM32WBX target config drifted")

    bridge = [row for row in rows if row["mapping_status"] == "deterministic_cmsis_commercial_bridge"]
    req({row["icpn"] for row in bridge} == EXPECTED_CMSIS_BRIDGE_ICPNS, "STM32WBX CMSIS bridge identity set drifted")
    req(all(
        row["existing_identifier_kind"] == "cmsis_device_name"
        and row["cmsis_device_name"]
        and row["cmsis_device_name"] == row["existing_identifier"]
        for row in bridge
    ), "STM32WBX CMSIS bridge semantics drifted")
    ...
    req(len(family_sources) == 1, "STM32WBX Production source binding missing or duplicated")
    req(family_sources[0] == {
    ...
    }, "STM32WBX Production source binding drifted")
    ...
    req(proposal.get("transaction") == "stm32wbx-production-catalog-publication", "publication transaction drifted")
    ...
    req(proposal.get("route_bridge_count") == 9, "proposal route bridge count drifted")
    ...
    for key in (
        ...
        req(proposal.get(key) is False, f"unsafe STM32WBX publication claim enabled: {key}")
    ...
    print("STM32WBX Production publication validation: PASS")
    return 0
