#!/usr/bin/env python3
"""Browser transport for STM32F3 dual-surface ST evidence.

This is intentionally family-specific. It reuses the same Chromium transport
constraints as the generic ST browser adapter, but waits for the STM32F3
Q&R-identity + Sample&Buy-lifecycle contract instead of changing the generic
readiness contract used by retained STM32F1/F2/F4 evidence.
"""
...
from stm32f3_dual_surface_evidence import dual_surface_ready

DEFAULT_NAVIGATION_ATTEMPTS = 2


class STM32F3BrowserAcquirer:
    def __init__(
        self,
        *,
        ...
        if not base_by_url:
            raise AcquisitionError("STM32F3 browser acquisition requires URL/Base bindings")
    ...
    def __enter__(self) -> "STM32F3BrowserAcquirer":
        try:
            ...
            raise AcquisitionError(
                "Playwright is required for STM32F3 browser acquisition"
            ) from exc
    ...
    def fetch(self, source_url: str, timeout_seconds: float) -> tuple[bytes, str, None, None]:
        ...
        if base_device is None:
            raise AcquisitionError("unregistered STM32F3 acquisition URL")
        ...
        for attempt in range(1, self.navigation_attempts + 1):
            ...
            try:
                ...
                while True:
                    ...
                    if remaining <= 0:
                        raise AcquisitionError(
                            "STM32F3 dual-surface evidence readiness timed out: "
