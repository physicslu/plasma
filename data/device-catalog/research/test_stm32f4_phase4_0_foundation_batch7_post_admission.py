...
EVIDENCE = HERE / "evidence" / "stm32f4-phase4.0-foundation-batch7-live-2026-08-31"
BASELINE = HERE / "stm32f4-phase4.0-foundation-batch7-baseline.json"
CATALOG = HERE / "openocd-parts-canonical.csv"
CANONICAL = HERE / "stm32f4-commercial-icpn.csv"
AUDIT = HERE / "stm32f4-phase4.0-foundation-batch7-admission-audit.json"
NEW_BASES = {"STM32F439ZG", "STM32F439ZI"}
EXPECTED_CANONICAL_SHA256 = "f7fbf1d8b6bcc7728f20dcb30c0b261e4b315459f05f76d6123308621f66a0a7"
EXPECTED_PREWRITE_PLAN_SHA256 = "60866cf5ccc91952efea82953efca64137273cef614c173fa5ae00e666cff288"
EXPECTED_CANONICAL_GIT_BLOB = "34ad23b2e689bab51b1819ae015e0d334f63cd85"
CONTROL_ICPNS = {"STM32F439VGT6"}
PRE_BATCH7_EVIDENCE_IDS = {
...
class STM32F4Phase40FoundationBatch7PostAdmissionTests(unittest.TestCase):
    ...
    def _prewrite_canonical(self, output: Path) -> None:
        with CANONICAL.open(newline="", encoding="utf-8") as handle:
            ...
            rows = [row for row in reader if _row_evidence_id(row) in PRE_BATCH7_EVIDENCE_IDS]
    ...
    def test_admission_audit_binds_read_only_proposal(self) -> None:
        ...
        self.assertEqual(audit["pre_rows"], 133)
        self.assertEqual(audit["new_exact_icpns"], 7)
        self.assertEqual(audit["post_rows"], 140)
        self.assertEqual(
            audit["decision_counts"],
            {"admit": 7, "already_present": 0, "manual_review_required": 0, "reject": 0},
        )
        ...
        self.assertEqual(audit["proposal_workflow_run_id"], "33371127532")
        self.assertEqual(audit["proposal_artifact_id"], "9750084244")
    ...
    def test_retained_evidence_is_scale_ready_and_has_no_non_active_rows(self) -> None:
        ...
        self.assertEqual(report["exact_icpn_candidates"], 8)
    ...
    def test_current_state_replans_batch7_as_already_present(self) -> None:
        ...
        self.assertEqual(
            ...
            {"admit": 0, "already_present": 7, "manual_review_required": 0, "reject": 0},
        )
        self.assertEqual(plan["conflicts"], 0)
        self.assertEqual(plan["issues"], [])

    def test_materialization_replays_133_to_140_and_matches_historical_hash(self) -> None:
        new_icpns = self._baseline_new_icpns()
        self.assertEqual(len(new_icpns), 7)
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(plan["canonical_rows_before"], 133)
            self.assertEqual(plan["candidate_count"], 7)
            self.assertEqual(plan["decision_counts"]["admit"], 7)
            self.assertTrue(pipeline_plan_is_clean(plan))

            first = write_canonical_dataset(plan=plan, canonical_path=canonical)
            self.assertEqual(first["status"], "written")
            self.assertEqual(first["rows_before"], 133)
            self.assertEqual(first["rows_after"], 140)
            self.assertEqual(set(first["added"]), new_icpns)

            second = write_canonical_dataset(plan=plan, canonical_path=canonical)
            self.assertEqual(second["status"], "no_op")
            self.assertEqual(second["rows_before"], 140)
            self.assertEqual(second["rows_after"], 140)
    ...
    def test_new_icpns_and_control_are_present(self) -> None:
        ...
        self.assertEqual(rows["STM32F439VGT6"]["base_device"], "STM32F439VG")
