#!/usr/bin/env python3
"""Controlled STM32L4 L4.5 canonical + Production publication transaction.

L4.5 publishes only the 446 exact ICPNs frozen as capability-admittable by
L4.4. Publication changes Device Catalog identity availability only. It does
...
from stm32l4_metadata_policy import build_candidate_inputs
from stm32l4_phase_l4_1_foundation import DEFAULT_CATALOG, read_catalog
from stm32l4_phase_l4_2_discovery import resolve_mapping

HERE = Path(__file__).resolve().parent
PRODUCTION_MANIFEST = HERE.parent / "production" / "icpn-v1-manifest.json"
PRESTATE_MANIFEST = HERE / "stm32-post-l0-production-manifest-prestate.json"
PLAN_PATH = HERE / "stm32l4-phase-l4.4-admission-plan.json"
CANONICAL_PATH = HERE / "stm32l4-commercial-icpn.csv"
PROPOSAL_PATH = HERE / "stm32l4-phase-l4.5-publication-proposal.json"
AUDIT_PATH = HERE / "stm32l4-phase-l4.5-publication-audit.json"
BASELINE_PATH = HERE / "stm32l4-phase-l4.5-publication-baseline.json"

PHASE = "L4.5"
FAMILY = "STM32L4"
EXPECTED_PLAN_GIT_BLOB = "ec660436dd6866abca28a63d6bac774f9ab50e49"
EXPECTED_PRESTATE_MANIFEST_SHA256 = "c435551f65cede76356ba45fa8523259d6201c2cc673c05ccf214a888beeefec"
EXPECTED_PRESTATE_MANIFEST_BLOB = "4e6a53695e86729063acd8ae102f66cc7eeb06c8"
EXPECTED_MAPPING_CATALOG_GIT_BLOB = "0ef056e3363e20bb527590c4a4cc1cc0d7afb810"
EXPECTED_EXACT_SET_SHA256 = "cdb350bdf1513f51430808b98677948569740d230c45bd735f439142ab03eb45"
EXPECTED_PUBLISHED_ROWS = 446
...
def _validate_frozen_inputs() -> dict[str, Any]:
    if _git_blob_sha(PLAN_PATH.read_bytes()) != EXPECTED_PLAN_GIT_BLOB:
        raise RuntimeError("frozen L4.4 admission-plan bytes drifted")
    frozen = _read_json(PLAN_PATH)
    if (
        frozen.get("phase") != "L4.4"
        ...
        or frozen.get("current_mapping_replay") != {"ambiguous": 0, "unique": 446, "unmapped": 0}
...
def _historical_canonical_rows() -> list[dict[str, str]]:
    ...
    for source_candidate in build_candidate_inputs():
        ...
        if not isinstance(icpn, str) or icpn in identities:
            raise RuntimeError(f"invalid/duplicate retained L4 candidate: {icpn}")
        identities.add(icpn)
    ...
    if len(rows) != EXPECTED_PUBLISHED_ROWS or len(identities) != EXPECTED_PUBLISHED_ROWS:
        raise RuntimeError("L4.5 canonical render cardinality drifted")
    if len({row["base_device"] for row in rows}) != EXPECTED_PUBLISHED_BASES:
        raise RuntimeError("L4.5 Base Device cardinality drifted")
    if _set_sha(identities) != EXPECTED_EXACT_SET_SHA256:
        raise RuntimeError("L4.5 exact published set drifted from L4.4")
    if any(row["family"] != FAMILY for row in rows):
        raise RuntimeError("L4.5 canonical render contains foreign-family row")
    return rows
...
def _build_post_manifest(canonical_bytes: bytes) -> bytes:
    ...
    if any(isinstance(source, dict) and source.get("family") == FAMILY for source in sources):
        raise RuntimeError("STM32L4 unexpectedly exists in frozen Production prestate")
    sources.append({
...
def build_proposal() -> dict[str, Any]:
    ...
    if FAMILY in before_families:
        raise RuntimeError("STM32L4 unexpectedly exists in Production prestate")
...
def _build_audit(proposal: dict[str, Any], canonical_bytes: bytes, manifest_bytes: bytes) -> dict[str, Any]:
    return {
        ...
        "added_exact_icpn_set_sha256": EXPECTED_EXACT_SET_SHA256,
        "stm32l4_rows_before": 0,
        "stm32l4_rows_after": EXPECTED_PUBLISHED_ROWS,
        "stm32l4_base_devices_after": EXPECTED_PUBLISHED_BASES,
        "production_exact_icpns_before": EXPECTED_PRESTATE[0],
...
def _build_baseline(proposal_bytes: bytes, audit_bytes: bytes, canonical_bytes: bytes, manifest_bytes: bytes) -> dict[str, Any]:
    return {
        ...
        "production_manifest_git_blob_sha_after": _git_blob_sha(manifest_bytes),
        "production_manifest_sha256_after": hashlib.sha256(manifest_bytes).hexdigest(),
        "production_prestate": {"exact_icpns": 1272, "base_devices": 392, "families": 11, "stm32l4": 0},
        "production_poststate": {"exact_icpns": 1718, "base_devices": 530, "families": 12, "stm32l4": 446},
        "claims": {
            "programming_algorithm_equivalence_claimed": False,
            "flash_geometry_equivalence_claimed": False,
            "option_security_semantics_claimed": False,
...
def verify_current_publication() -> dict[str, Any]:
    ...
    l4_sources = [source for source in sources if isinstance(source, dict) and source.get("family") == FAMILY]
    if len(l4_sources) != 1:
        raise RuntimeError("current Production manifest must contain exactly one STM32L4 source")
    expected_source = {
        "manufacturer": "STMicroelectronics",
        "family": FAMILY,
        "path": "../research/stm32l4-commercial-icpn.csv",
    ...
    return {
        ...
        "stm32l4_production": families.get(FAMILY),
    }
...
def publish() -> dict[str, Any]:
    ...
    if PRODUCTION_MANIFEST.read_bytes() != PRESTATE_MANIFEST.read_bytes():
        raise RuntimeError("current Production manifest drifted from L4.5 frozen prestate")
