#!/usr/bin/env python3
"""Bounded, fail-closed official-ST discovery for STM32G0 Phase 4.8B.
...
from Phase 4.8A are routing/name metadata only and never participate in exact
ICPN selection.
"""
...
from stm32g0_dual_surface_evidence import build_dual_surface_browser_evidence_record
...
PHASE = "4.8B"
DEFAULT_MANIFEST = HERE / "stm32g0-phase4.8b-discovery-manifest.json"
DEFAULT_OUTPUT = Path("/tmp/stm32g0-phase4.8b-live-summary.json")
...
def read_manifest(path: Path, catalog_rows: list[dict[str, str]]) -> tuple[str, list[DiscoveryTarget]]:
    ...
    if payload.get("schema_version") != 1 or payload.get("phase") != PHASE:
        raise AcquisitionError("unsupported STM32G0 Phase 4.8B discovery manifest")
    pilot_id = payload.get("pilot_id")
    raw_targets = payload.get("targets")
    if not isinstance(pilot_id, str) or not pilot_id.strip():
        raise AcquisitionError("Phase 4.8B discovery manifest requires pilot_id")
    if not isinstance(raw_targets, list) or len(raw_targets) != MAX_TARGETS:
        raise AcquisitionError(f"Phase 4.8B discovery requires exactly {MAX_TARGETS} targets")

    targets: list[DiscoveryTarget] = []
    for index, raw in enumerate(raw_targets, start=1):
        if not isinstance(raw, dict):
            raise AcquisitionError(f"Phase 4.8B target {index} must be an object")
        ...
        if not isinstance(subfamily, str) or subfamily not in EXPECTED_SUBFAMILIES:
            raise AcquisitionError(f"invalid STM32G0 subfamily: {subfamily!r}")
        if not isinstance(base, str):
            raise AcquisitionError(f"invalid STM32G0 Base Device: {base!r}")
    ...
    if observed != expected:
        raise AcquisitionError(
            f"Phase 4.8B target selection drifted: expected={expected} observed={observed}"
        )
...
def run_discovery(
    *,
    ...
    return {
        ...
        "family": FAMILY,
        "pilot_id": pilot_id,
        "scope": "bounded read-only official-ST STM32G0 commercial identity/lifecycle discovery",
