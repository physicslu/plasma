...
DEFAULT_EVIDENCE_DIR = (
    HERE / "evidence/stm32f0-phase4.5b-official-st-discovery-live-2026-09-09"
)
...
EXPECTED_BASELINE_SHA256 = "d0987137c0c9336d3a36c9108409e73a9cd403bc82c9133b41337bcaeb32e57f"
...
EXPECTED_ARTIFACT_ZIP_SHA256 = (
    "a3c02afd6a1295a612d25e35031d8623da86c0ea76777acaab4caaaab51cb1aa"
)
EXPECTED_LIVE_SUMMARY_SHA256 = (
    "e96c9f09f4f57d2797cd9e3928664dcd67c72f3887eae3795df66aa1e75cbb76"
)
...
def validate(
    *,
    ...
    require(
        sha256(evidence_dir / "pilot-summary.json") == EXPECTED_SUMMARY_SHA256,
        "retained summary byte digest mismatch",
    )
    ...
    require(baseline.get("phase") == "4.5B", "baseline phase mismatch")
    require(baseline.get("family") == "STM32F0", "baseline family mismatch")
    require(summary.get("schema_version") == 1, "summary schema mismatch")
    require(summary.get("phase") == "4.5B", "summary phase mismatch")
    require(summary.get("family") == "STM32F0", "summary family mismatch")
    require(
        summary.get("pilot_id") == baseline.get("pilot_id") == discovery_manifest.get("pilot_id"),
        "pilot identity mismatch",
    )

    execution = baseline.get("discovery_execution")
    browser = baseline.get("browser")
    require(isinstance(execution, dict), "baseline discovery execution missing")
    require(isinstance(browser, dict), "baseline browser binding missing")
    require(execution.get("head_git_sha") == EXPECTED_EXECUTED_GIT_SHA, "baseline head SHA mismatch")
    require(execution.get("github_checkout_sha") == EXPECTED_CHECKOUT_SHA, "baseline checkout SHA mismatch")
    require(execution.get("workflow_run_id") == EXPECTED_RUN_ID, "baseline workflow run mismatch")
    require(execution.get("artifact_id") == EXPECTED_ARTIFACT_ID, "baseline artifact mismatch")
    require(execution.get("artifact_zip_sha256") == EXPECTED_ARTIFACT_ZIP_SHA256, "baseline artifact digest mismatch")
    require(browser == {
        "browser_version": "151.0.7922.34",
        "evidence_profile": EXPECTED_PROFILE,
        "headless": False,
        "playwright_version": "1.62.0",
    }, "baseline browser metadata mismatch")
    ...
    require(provenance.get("workflow_run_id") == EXPECTED_RUN_ID, "provenance workflow run mismatch")
    ...
    require(core["exact_icpn_candidate_count"] == EXPECTED_EXACT_COUNT, "provenance exact ICPN count mismatch")
    ...
    # The discovery manifest is itself an immutable Phase 4.5B input, so its
    # current repository bytes must remain the bytes used by the live run.
    require(
        git_blob_sha(discovery_manifest_path) == EXPECTED_DISCOVERY_MANIFEST_GIT_BLOB_SHA,
        "Phase 4.5B discovery manifest drifted",
    )
    require(discovery_manifest.get("schema_version") == 1, "discovery manifest schema mismatch")
    require(discovery_manifest.get("phase") == "4.5B", "discovery manifest phase mismatch")
    ...
    require(aggregate == expected_aggregate, "baseline aggregate result mismatch")
    ...
    }, "retained summary aggregate mismatch")
    ...
    require(isinstance(manifest_targets, list) and len(manifest_targets) == 13, "discovery manifest target list mismatch")
    ...
    require(observed_pairs == expected_pairs, "discovery target ordering drifted")
    ...
    for baseline_target, manifest_target in zip(baseline_targets, manifest_targets):
        ...
        require(
            manifest_target.get("source_url") == baseline_target.get("source_url"),
            f"{base}: source URL drifted",
        )
        require(
            baseline_target.get("source_url")
            == f"https://www.st.com/en/microcontrollers-microprocessors/{base.lower()}.html",
            f"{base}: non-canonical ST source URL",
        )
        exact = baseline_target.get("exact_icpns")
        require(isinstance(exact, list) and exact, f"{base}: exact ICPNs missing")
        require(all(isinstance(value, str) and value.startswith(base) for value in exact), f"{base}: foreign ICPN in baseline")
        ...
        retrieved = baseline_target.get("retrieved_at_utc")
        require(isinstance(retrieved, str) and retrieved.startswith("2026-09-09T"), f"{base}: retrieval timestamp mismatch")
    ...
    # Deliberately do NOT read current Production or current OpenOCD mapping
    # here. Their historical Git-blob identities are retained above, but future
    # catalog/capability evolution must not invalidate manufacturer-backed
    # commercial identity established by this transaction.
