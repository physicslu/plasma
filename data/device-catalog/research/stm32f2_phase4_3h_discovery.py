#!/usr/bin/env python3
"""Run the third bounded, fail-closed official-ST STM32F2 discovery."""
...
DEFAULT_MANIFEST = HERE / "stm32f2-phase4.3h-discovery-manifest.json"
PHASE = "4.3H"
...
EXPECTED_PRODUCTION_SHA256 = "1706ab65dccb112a7ff907d82cf07c5ef6a46110097746aaeb5f3f9aed17c5cb"
...
def deterministic_targets(
    ...
    if tuple(sorted(by_subfamily)) != EXPECTED_SUBFAMILIES:
        raise AcquisitionError("Phase 4.3H unadmitted STM32F2 subfamily set drifted")
...
def read_manifest(
    ...
    if payload.get("schema_version") != 1 or payload.get("phase") != PHASE:
        raise AcquisitionError("unsupported STM32F2 Phase 4.3H discovery manifest")
    pilot_id = payload.get("pilot_id")
    raw_targets = payload.get("targets")
    if not isinstance(pilot_id, str) or not pilot_id.strip():
        raise AcquisitionError("Phase 4.3H discovery manifest requires pilot_id")
    if not isinstance(raw_targets, list) or len(raw_targets) != MAX_TARGETS:
        raise AcquisitionError(f"Phase 4.3H discovery requires exactly {MAX_TARGETS} targets")

    targets: list[DiscoveryTarget] = []
    for raw in raw_targets:
        if not isinstance(raw, dict):
            raise AcquisitionError("Phase 4.3H target must be an object")
    ...
    if observed != expected:
        raise AcquisitionError(f"Phase 4.3H target selection drifted: expected={expected} observed={observed}")
...
def run_discovery(**kwargs: Any) -> dict[str, object]:
    ...
    summary["phase"] = PHASE
    summary["scope"] = "third bounded read-only official-ST STM32F2 discovery"
    return summary
