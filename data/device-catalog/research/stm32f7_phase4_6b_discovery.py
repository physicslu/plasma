#!/usr/bin/env python3
"""Bounded, fail-closed official-ST discovery for STM32F7 Phase 4.6B.
...
from stm32f7_dual_surface_evidence import build_dual_surface_browser_evidence_record
...
PHASE = "4.6B"
DEFAULT_MANIFEST = HERE / "stm32f7-phase4.6b-discovery-manifest.json"
...
def read_manifest(
    ...
    if payload.get("schema_version") != 1 or payload.get("phase") != PHASE:
        raise AcquisitionError("unsupported STM32F7 Phase 4.6B discovery manifest")
    pilot_id = payload.get("pilot_id")
    raw_targets = payload.get("targets")
    if not isinstance(pilot_id, str) or not pilot_id.strip():
        raise AcquisitionError("Phase 4.6B discovery manifest requires pilot_id")
    if not isinstance(raw_targets, list) or len(raw_targets) != MAX_TARGETS:
        raise AcquisitionError(
            f"Phase 4.6B discovery requires exactly {MAX_TARGETS} targets"
        )

    targets: list[DiscoveryTarget] = []
    for index, raw in enumerate(raw_targets, start=1):
        if not isinstance(raw, dict):
            raise AcquisitionError(f"Phase 4.6B target {index} must be an object")
        ...
        if not isinstance(subfamily, str) or subfamily not in EXPECTED_SUBFAMILIES:
            raise AcquisitionError(f"invalid STM32F7 subfamily: {subfamily!r}")
        if not isinstance(base, str):
            raise AcquisitionError(f"invalid STM32F7 Base Device: {base!r}")
    ...
    if observed != expected:
        raise AcquisitionError(
            f"Phase 4.6B target selection drifted: expected={expected} observed={observed}"
        )
