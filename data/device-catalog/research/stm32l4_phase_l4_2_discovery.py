#!/usr/bin/env python3
"""Fail-closed STM32L4 L4.2 manufacturer-authoritative commercial discovery.

L4.2 expands the frozen L4.1 ordering-pattern surface into the complete
bounded deterministic Base Device research set, then observes exact commercial
ICPN identity and lifecycle only from official ST dual-surface evidence.

Quality & Reliability is exact Part Number identity authority. Sample & Buy is
Marketing Status authority. Exact sets must join before lifecycle disposition is
accepted. OpenOCD bounds research and supplies routing diagnostics only.
...
from stm32l4_phase_l4_1_foundation import (
    DEFAULT_BASELINE as L4_1_BASELINE,
    DEFAULT_CATALOG,
...
PHASE = "L4.2"
FAMILY = "STM32L4"
PARSER_PROFILE = "stm32l4_l4_2_dual_surface_v1"
DISCOVERY_ID = "stm32l4-l4.2-official-st-commercial-discovery-v1"
EXPECTED_L4_1_BASELINE_SHA256 = "3f85587e74f9af1ee93f59d53a05ea1d47201e6b8564b8e8a91cb91f378bd40f"
EXPECTED_L4_1_REPRESENTATIVES = {base for _subfamily, base in EXPECTED_TARGETS}
...
DEFAULT_OUTPUT = Path("/tmp/stm32l4-l4.2-live-summary.json")
MAX_TARGETS = 207
...
def validate_l4_1_boundary(path: Path = L4_1_BASELINE) -> dict[str, Any]:
    if sha256(path) != EXPECTED_L4_1_BASELINE_SHA256:
        raise AcquisitionError("L4.1 foundation baseline SHA-256 drifted")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict) or payload.get("phase") != "L4.1" or payload.get("family") != FAMILY:
        raise AcquisitionError("unexpected L4.1 foundation baseline identity")
    if payload.get("source_row_count") != 255 or payload.get("target_config") != TARGET_CONFIG:
        raise AcquisitionError("L4.1 bounded research surface drifted")
    if payload.get("identifier_kind_counts") != {"cmsis_device_name": 48, "ordering_pattern": 207}:
        raise AcquisitionError("L4.1 identifier-kind boundary drifted")
    _all_false(payload.get("claims"), "L4.1 claims")
    reps = payload.get("initial_targets")
    observed = {item.get("base_device") for item in reps or [] if isinstance(item, dict)}
    if observed != EXPECTED_L4_1_REPRESENTATIVES:
        raise AcquisitionError("L4.1 representative continuity boundary drifted")
    return payload
...
def deterministic_targets(catalog_rows: list[dict[str, str]]) -> list[DiscoveryTarget]:
    ...
    for subfamily in EXPECTED_SUBFAMILIES:
        ...
        for base in bases:
            targets.append(DiscoveryTarget(
                ...
                selection_reason=(
                    "deterministic unique Base Device collapsed from frozen L4.1 "
    ...
    if not bases or len(bases) > MAX_TARGETS or len(set(bases)) != len(bases):
        raise AcquisitionError(f"L4.2 invalid deterministic Base Device set: {len(bases)}")
    if not EXPECTED_L4_1_REPRESENTATIVES.issubset(set(bases)):
        raise AcquisitionError("L4.2 deterministic set lost a frozen L4.1 representative")
    return targets


def validate_targets(targets: list[DiscoveryTarget]) -> None:
    if not targets or len(targets) > MAX_TARGETS:
        raise AcquisitionError("L4.2 target count escaped frozen ordering-pattern bound")
    seen: set[str] = set()
    covered: set[str] = set()
    for target in targets:
        if target.subfamily not in EXPECTED_SUBFAMILIES:
            raise AcquisitionError(f"unsupported L4 subfamily: {target.subfamily}")
        if target.base_device in seen or not target.base_device.startswith(target.subfamily):
            raise AcquisitionError(f"invalid/duplicate L4 Base Device: {target.base_device}")
    ...
    if covered != set(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("L4.2 target manifest lost a frozen subfamily")
    if not EXPECTED_L4_1_REPRESENTATIVES.issubset(seen):
        raise AcquisitionError("L4.2 target manifest lost L4.1 representatives")
...
def target_manifest(targets: list[DiscoveryTarget]) -> dict[str, object]:
    ...
    return {
        ...
        "scope": "bounded read-only official-ST STM32L4 complete commercial discovery",
...
def build_discovery_evidence_record(**kwargs: Any) -> dict[str, object]:
    ...
    if evidence.get("evidence_surface") != AUTHORITY_SURFACE:
        raise AcquisitionError("STM32L4 L4.2 evidence authority surface drifted")
    return evidence
    ...
    *,
    ...
    for target in targets:
        ...
        try:
            ...
            for icpn in active:
                owner = exact_owner.setdefault(icpn, target.base_device)
    ...
    representative_results = {
        ...
        if result.get("base_device") in EXPECTED_L4_1_REPRESENTATIVES
    }
    representative_continuity = (
        set(representative_results) == EXPECTED_L4_1_REPRESENTATIVES
...
def write_evidence_files(summary: dict[str, object], evidence_dir: Path) -> None:
    ...
    for result in summary.get("results", []):
        ...
        if not isinstance(base, str):
            continue
        path = evidence_dir / f"{base.lower()}.json"
