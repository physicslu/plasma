#!/usr/bin/env python3
"""Post-publication hard-lock regressions for STM32L1 L1.5."""
...
class STM32L1PhaseL15PublicationTests(unittest.TestCase):
    @classmethod
    ...
    def test_publication_state_is_valid(self) -> None:
        ...
        self.assertEqual(summary["stm32l1_production"], EXPECTED_PUBLISHED_ROWS)
    ...
    def test_canonical_rows_retain_l1_admission_semantics(self) -> None:
        ...
        self.assertTrue(all(row["openocd_target_config"] == "tcl/target/stm32l1.cfg" for row in self.rows))
        statuses = {row["verification_status"] for row in self.rows}
        self.assertEqual(statuses, {CANONICAL_VERIFICATION_STATUS})
        self.assertTrue(all(status.startswith("verified_") for status in statuses))

    def test_production_manifest_has_one_bound_l1_source(self) -> None:
        ...
        self.assertEqual(source["path"], "../research/stm32l1-commercial-icpn.csv")
