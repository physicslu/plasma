#!/usr/bin/env python3
"""Post-C0 STM32 next-family research selection policy.

Consumes retained official-ST identity/lifecycle evidence and an independent
Ordering Information review. This transaction may select only the next family
for discovery research. It never admits devices or authorizes Production.
"""
...
from stm32_post_c0_evidence_probe import EXPECTED_SHORTLIST, EXPECTED_SURFACES, EXPECTED_TARGET_COUNT
...
SELECTION_ID = "stm32-post-c0-next-family-selection-v1"
...
def validate_authoritative_summary(summary: dict[str, Any]) -> None:
    if summary.get("schema_version") != 1 or summary.get("phase") != "post-C0-selection":
        raise AcquisitionError("unsupported post-C0 evidence summary")
    if tuple(summary.get("candidate_series", [])) != EXPECTED_SHORTLIST:
        raise AcquisitionError("post-C0 evidence candidate order drifted")
    if summary.get("attempted_targets") != EXPECTED_TARGET_COUNT:
        raise AcquisitionError("post-C0 evidence target count drifted")
    ...
    for result in results:
        ...
        if not isinstance(evidence, dict) or evidence.get("evidence_surface") != AUTHORITY_SURFACE:
            raise AcquisitionError(f"{result.get('base_device')}: unsupported commercial evidence surface")
        ...
        disposition_ids = list(exact) + [row.get("icpn") for row in excluded if isinstance(row, dict)]
        if len(record_ids) != len(set(record_ids)) or set(record_ids) != set(disposition_ids):
            raise AcquisitionError(f"{result.get('base_device')}: joined lifecycle set does not equal exact identity set")
...
def build_selection(summary: dict[str, Any], ordering_review: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(dispositions[series] == "evidence_incomplete" for series in EXPECTED_SHORTLIST):
        ...
        if reviewed != eligible:
            raise AcquisitionError(f"Ordering Information comparison set drifted: expected {eligible}, got {reviewed}")
        ordering_result = comparison.get("result")
        if ordering_result != "equivalent_required_ordering_evidence_quality":
            selected = None
    ...
    return {
        ...
        "tie_break_policy": "current_post_c0_cross_family_prioritization_order_after_equivalent_required_evidence",
