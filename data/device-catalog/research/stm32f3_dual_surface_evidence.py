#!/usr/bin/env python3
"""STM32F3-specific dual-surface official-ST evidence extraction.

Some STM32F3 product pages expose exact orderable part numbers in the Quality
and Reliability table but expose Marketing Status only in the Sample & Buy
table. This adapter keeps those responsibilities separate and joins them only
when the exact commercial identity sets match byte-for-byte as logical sets.

It deliberately does not change the generic ST acquisition parser used by
previously retained STM32F1/F2/F4 evidence.
"""
...
class SectionTableParser(HTMLParser):
    """Collect visible-to-parser text/table rows under one exact H2 heading."""
...
def _parse_section(html_text: str, heading: str) -> SectionTableParser:
...
def extract_dual_surface_part_number_records(
    ...
    """Join Q&R exact identity to Sample & Buy Marketing Status fail-closed."""

    if re.fullmatch(r"STM32[A-Z0-9]+", base_device) is None:
        raise AcquisitionError(f"invalid STM32 base device: {base_device!r}")

    quality = _parse_section(html_text, QUALITY_HEADING)
    sample = _parse_section(html_text, SAMPLE_BUY_HEADING)
    if not quality.seen:
    ...
    sample_text = normalize_text(" ".join(sample.section_parts))
    quality_foreign = [
        token
    ...
    if quality_set != lifecycle_set:
        ...
        raise AcquisitionError(
            "STM32F3 dual-surface exact ICPN set mismatch: "
...
def build_dual_surface_browser_evidence_record(
    *,
    ...
    records, canonical_text = extract_dual_surface_part_number_records(
        html_text,
        base_device,
    )
    ...
    return {
        ...
        "parser_profile": "stm32f3_dual_surface_v1",
