#!/usr/bin/env python3
"""Bounded official-ST evidence accessibility probe for STM32H7RS.

This transaction proves only that one deterministic representative Base Device
per frozen H7R/S subfamily has accessible official ST commercial-identity and
lifecycle evidence. It does not perform full exact-ICPN discovery, admit devices,
write Production, define programming equivalence, or claim runtime/HIL support.
"""
...
SELECTION_PATH = HERE / "stm32h7-partitioned-scope-selection.json"
PROBE_ID = "stm32h7rs-bounded-official-st-evidence-accessibility-v1"
EXPECTED_TARGET_CONFIG = "tcl/target/stm32h7rsx.cfg"
EXPECTED_FAMILY = "STM32H7RS Series"
EXPECTED_SUBFAMILIES = ("STM32H7R3", "STM32H7R7", "STM32H7S3", "STM32H7S7")
EXPECTED_ROWS = 34
EXPECTED_ORDERING_ROWS = 30
EXPECTED_CMSIS_ROWS = 4
EXPECTED_DUPLICATE_RESOLUTIONS = 34
...
def guarded_partition_rows() -> list[dict[str, str]]:
    ...
    if selection.get("selection_id") != "stm32h7-partitioned-scope-selection-v1":
        raise AcquisitionError("unexpected STM32H7 partition selection id")
    if selection.get("selected_partition") != "STM32H7RS":
        raise AcquisitionError("STM32H7RS is no longer the selected bounded partition")
    if selection.get("selected_target_config") != EXPECTED_TARGET_CONFIG:
        raise AcquisitionError("STM32H7RS selected target config drifted")
    if selection.get("next_gate") != "stm32h7rs-bounded-official-manufacturer-evidence-accessibility-gate":
        raise AcquisitionError("STM32H7RS evidence-accessibility gate drifted")
    ...
    if not isinstance(resolutions, list) or len(resolutions) != EXPECTED_DUPLICATE_RESOLUTIONS:
    ...
    if len(bounded) != EXPECTED_ROWS:
        raise AcquisitionError(f"STM32H7RS bounded row count drifted: {len(bounded)}")
    ...
    if kinds != Counter({"ordering_pattern": EXPECTED_ORDERING_ROWS, "cmsis_device_name": EXPECTED_CMSIS_ROWS}):
        raise AcquisitionError(f"STM32H7RS identifier-kind surface drifted: {dict(kinds)}")
    subfamilies = tuple(sorted({row.get("subfamily", "") for row in bounded}))
    if subfamilies != EXPECTED_SUBFAMILIES:
        raise AcquisitionError(f"STM32H7RS subfamily surface drifted: {subfamilies}")
    if any(row.get("plasma_series") != "STM32H7" for row in bounded):
        raise AcquisitionError("STM32H7RS row escaped STM32H7 plasma-series boundary")
    return bounded
    ...
    for subfamily in EXPECTED_SUBFAMILIES:
        ...
        targets.append(
            ProbeTarget(
                ...
                base_device=base,
                source_url=url,
                selection_reason="deterministic lexical-min Base Device in frozen STM32H7RS ordering-pattern subfamily",
            )
        )
    if len(targets) != len(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("STM32H7RS representative target count drifted")
    return targets
...
def run_live(targets: list[ProbeTarget], *, headless: bool, delay_seconds: float, timeout_seconds: float) -> tuple[dict[str, object], str | None]:
    ...
    summary = {
        "schema_version": 1,
        "probe_id": PROBE_ID,
        "scope": "STM32H7RS bounded representative official-ST commercial identity/lifecycle evidence accessibility",
        ...
        "next_gate": "stm32h7rs-bounded-exact-icpn-discovery-gate" if complete else None,
        "claims": {
...
def main() -> None:
    ...
    if summary.get("bounded_probe_complete") is not True:
        raise SystemExit("STM32H7RS evidence accessibility probe requires manual review")
