#!/usr/bin/env python3
"""Phase 4.5D deterministic STM32F0 admission planner.

Requires the closed Phase 4.5C metadata policy, independently resolves current
...
Production publication are separate controlled transactions.
"""
...
from stm32f0_admission_policy import CANONICAL_FIELDS, FAMILY, build_canonical_row
from stm32f0_foundation import DEFAULT_CATALOG, read_catalog
from stm32f0_metadata_policy import build_candidate_inputs
...
ADAPTER_ID = "stm32f0-phase4.5d-admission"
DEFAULT_CANONICAL = HERE / "stm32f0-commercial-icpn.csv"


class STM32F0AdmissionError(AdmissionError):
    pass
...
def _canonical_prestate(canonical_path: Path | None) -> tuple[list[str], list[dict[str, str]], bool]:
    ...
    if tuple(fields) != CANONICAL_FIELDS:
        raise STM32F0AdmissionError("STM32F0 canonical CSV schema mismatch")
    if any(row.get("family") not in ("", FAMILY) for row in rows):
        raise STM32F0AdmissionError("STM32F0 canonical CSV contains a foreign family")
    identities = [row.get("icpn", "") for row in rows]
    if len(identities) != len(set(identities)):
        raise STM32F0AdmissionError("STM32F0 canonical CSV contains duplicate ICPNs")
    if rows:
        raise STM32F0AdmissionError(
            f"Phase 4.5D requires zero-row STM32F0 canonical prestate, got {len(rows)}"
        )
...
def build_admission_plan(
    *,
    ...
    if not policy_plan_is_clean(metadata_plan):
        raise STM32F0AdmissionError("Phase 4.5C metadata policy is not clean")
    ...
    if not isinstance(evidence_id, str) or evidence_id != metadata_plan.get("evidence_id"):
        raise STM32F0AdmissionError("4.5B/4.5C evidence identity drifted")
    ...
    for candidate in candidate_inputs:
        ...
        if not isinstance(icpn, str):
            raise STM32F0AdmissionError("candidate lacks exact ICPN")
    ...
    plan.update(
        {
            ...
            "required_target_config": "tcl/target/stm32f0x.cfg",
...
def admission_plan_is_clean(plan: dict[str, Any]) -> bool:
    return (
        ...
        and plan.get("required_target_config") == "tcl/target/stm32f0x.cfg"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not admission_plan_is_clean(plan):
            raise STM32F0AdmissionError("Phase 4.5D admission plan is not clean")
