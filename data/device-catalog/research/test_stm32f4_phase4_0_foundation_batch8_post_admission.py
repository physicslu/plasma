...
AUDIT = HERE / "stm32f4-phase4.0-foundation-batch8-admission-audit.json"
NEW_BASES = {"STM32F417VE", "STM32F417ZE"}
EXPECTED_CANONICAL_SHA256 = "c37c2931bd27c5754a65cc3e1d7f14702637f95153941abc650b0b30d526eae8"
EXPECTED_PREWRITE_PLAN_SHA256 = "f4f79a0850d50f3ec7f09123e901d08a201eec7300ede4eed4a1db929d8e8b5a"
EXPECTED_CANONICAL_GIT_BLOB = "b59a46cc242d7d7921c287d8c656eab6e603e0d2"
CONTROL_ICPNS = {"STM32F417VGT6"}
PRE_BATCH8_EVIDENCE_IDS = {
...
class STM32F4Phase40FoundationBatch8PostAdmissionTests(unittest.TestCase):
    ...
    def _prewrite_canonical(self, output: Path) -> None:
        with CANONICAL.open(newline="", encoding="utf-8") as handle:
            ...
            rows = [row for row in reader if _row_evidence_id(row) in PRE_BATCH8_EVIDENCE_IDS]
    ...
    def test_admission_audit_binds_read_only_proposal(self) -> None:
        ...
        self.assertEqual(audit["pre_rows"], 140)
        self.assertEqual(audit["new_exact_icpns"], 3)
        self.assertEqual(audit["post_rows"], 143)
        self.assertEqual(
            audit["decision_counts"],
            {"admit": 3, "already_present": 0, "manual_review_required": 0, "reject": 0},
        )
        ...
        self.assertEqual(audit["proposal_workflow_run_id"], "33391164641")
        self.assertEqual(audit["proposal_artifact_id"], "9757487138")
    ...
    def test_current_state_replans_batch8_as_already_present(self) -> None:
        ...
        self.assertEqual(
            ...
            {"admit": 0, "already_present": 3, "manual_review_required": 0, "reject": 0},
        )
        self.assertEqual(plan["conflicts"], 0)
        self.assertEqual(plan["issues"], [])

    def test_materialization_replays_140_to_143_and_matches_historical_hash(self) -> None:
        new_icpns = self._baseline_new_icpns()
        self.assertEqual(len(new_icpns), 3)
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(plan["canonical_rows_before"], 140)
            self.assertEqual(plan["candidate_count"], 3)
            ...
            self.assertEqual(first["rows_before"], 140)
            self.assertEqual(first["rows_after"], 143)
            self.assertEqual(set(first["added"]), new_icpns)

            second = write_canonical_dataset(plan=plan, canonical_path=canonical)
            self.assertEqual(second["status"], "no_op")
            self.assertEqual(second["rows_before"], 143)
            self.assertEqual(second["rows_after"], 143)
    ...
    def test_new_icpns_and_control_are_present(self) -> None:
        ...
        self.assertEqual(rows["STM32F417VGT6"]["base_device"], "STM32F417VG")
