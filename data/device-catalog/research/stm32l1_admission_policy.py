"""STM32L1 L1.4 canonical admission row policy.

L1.3 owns manufacturer-authoritative commercial metadata. L1.4 adds one
independent OpenOCD ordering-pattern routing gate for read-only canonical
admission planning. A positive route is not a programming algorithm, Flash
geometry, security, electrical/socket, HIL, or runtime qualification claim.
"""
...
from stm32l1_metadata_policy import FAMILY, MANUFACTURER, METADATA_FIELDS, build_metadata_row
from stm32l1_phase_l1_1_foundation import TARGET_CONFIG
from stm32l1_phase_l1_2_discovery import commercial_core
...
def build_canonical_row(candidate: dict[str, Any], fields: list[str]) -> dict[str, str]:
    """Combine frozen L1.3 metadata with one strict STM32L1 OpenOCD route."""
    ...
    if (
        ...
        raise CandidateManualReview(
            f"{icpn}: exact ICPN lacks one unique STM32L1 OpenOCD ordering-pattern mapping"
        )
    ...
    if tuple(fields) != CANONICAL_FIELDS:
        raise AdmissionError("canonical CSV schema is not supported by STM32L1 L1.4")
    if values["manufacturer"] != MANUFACTURER or values["family"] != FAMILY:
        raise AdmissionError("STM32L1 metadata identity drifted before admission")
