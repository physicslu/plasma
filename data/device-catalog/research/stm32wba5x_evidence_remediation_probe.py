#!/usr/bin/env python3
"""Bounded official-ST evidence remediation probe for STM32WBA5X.

This transaction proves only that one deterministic representative Base Device
per frozen STM32WBA5X subfamily has accessible official ST commercial identity
...
SELECTION_PATH = HERE / "stm32-post-wbx-wireless-frontier-selection.json"
BLOCKED_GATE_PATH = HERE / "stm32wba5x-evidence-accessibility-gate.json"
EXHAUSTION_PATH = HERE / "stm32-wireless-frontier-exhaustion.json"

EXPECTED_SELECTION_GIT_BLOB_SHA = "849488b36d398a7e8f8da83b3aa93fbe320f8dc3"
EXPECTED_BLOCKED_GATE_GIT_BLOB_SHA = "26c0d55a4d0e1f85025134b0c9bda38049b5b3a4"
EXPECTED_EXHAUSTION_GIT_BLOB_SHA = "3345ead0c1f5fb0b103fc8fb2da84c24533b5a67"

PROBE_ID = "stm32wba5x-bounded-official-st-evidence-remediation-v1"
EXPECTED_SOURCE_SHA256 = "43ca9f9bbd2826aef8cd147a251263255d957dd1bcac7da653905d3bf980b6a3"
EXPECTED_TARGET_CONFIG = "tcl/target/stm32wba5x.cfg"
EXPECTED_FAMILY = "STM32WBA Series"
EXPECTED_SERIES = "STM32WBA5X"
EXPECTED_SUBFAMILIES = ("STM32WBA50", "STM32WBA52", "STM32WBA54", "STM32WBA55", "STM32WBA5M")
EXPECTED_ROWS = 34
EXPECTED_ORDERING_ROWS = 17
EXPECTED_CMSIS_ROWS = 17
...
def guarded_remediation_state() -> None:
    if git_blob_sha(SELECTION_PATH) != EXPECTED_SELECTION_GIT_BLOB_SHA:
        raise AcquisitionError("frozen STM32WBA5X selection Git blob drifted")
    if git_blob_sha(BLOCKED_GATE_PATH) != EXPECTED_BLOCKED_GATE_GIT_BLOB_SHA:
        raise AcquisitionError("frozen STM32WBA5X blocked gate Git blob drifted")
    if git_blob_sha(EXHAUSTION_PATH) != EXPECTED_EXHAUSTION_GIT_BLOB_SHA:
        raise AcquisitionError("frozen wireless exhaustion Git blob drifted")

    gate = load_json(BLOCKED_GATE_PATH)
    if (
        gate.get("gate_id") != "stm32wba5x-bounded-official-st-evidence-accessibility-v1"
        or gate.get("decision") != "defer_frontier_pending_official_product_evidence"
        or gate.get("catalog_admission_ready") is not False
    ):
        raise AcquisitionError("STM32WBA5X blocked evidence gate drifted")
    blocked = (gate.get("representative_results") or {}).get("STM32WBA50")
    if not isinstance(blocked, dict):
        raise AcquisitionError("STM32WBA50 blocked evidence missing")
    if (
        blocked.get("base_device") != "STM32WBA50KE"
        or blocked.get("status") != "blocked_manual_review"
        or blocked.get("error") != "browser navigation returned HTTP 404"
    ):
        raise AcquisitionError("STM32WBA50 retained blocker drifted")

    exhaustion = load_json(EXHAUSTION_PATH)
    if (
        exhaustion.get("status") != "no_currently_eligible_wireless_frontier"
        or exhaustion.get("selected_wireless_frontier") is not None
        or exhaustion.get("eligible_wireless_frontiers") != []
    ):
        raise AcquisitionError("wireless exhaustion boundary drifted")
    blockers = exhaustion.get("current_blockers")
    if not isinstance(blockers, list) or not any(
        isinstance(item, dict)
        and item.get("plasma_series") == "STM32WBA5X"
        and item.get("base_device") == "STM32WBA50KE"
        and item.get("blocker") == "official_st_product_page_http_404"
        for item in blockers
    ):
        raise AcquisitionError("STM32WBA5X exhaustion blocker missing")


def guarded_series_rows() -> list[dict[str, str]]:
    guarded_remediation_state()
    selection = load_json(SELECTION_PATH)
    if selection.get("selection_id") != "stm32-post-wbx-wireless-frontier-selection-v1":
        raise AcquisitionError("unexpected post-WBX wireless frontier selection id")
    if selection.get("selected_wireless_frontier") != EXPECTED_SERIES:
        raise AcquisitionError("STM32WBA5X is no longer the selected wireless frontier")
    if selection.get("selected_target_config") != EXPECTED_TARGET_CONFIG:
        raise AcquisitionError("STM32WBA5X selected target config drifted")
    if selection.get("selected_row_count") != EXPECTED_ROWS:
        raise AcquisitionError("STM32WBA5X selected row count drifted")
    if selection.get("selected_subfamily_count") != len(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("STM32WBA5X selected subfamily count drifted")
    if tuple(selection.get("selected_subfamilies") or ()) != EXPECTED_SUBFAMILIES:
        raise AcquisitionError("STM32WBA5X selected subfamilies drifted")
    if selection.get("selected_identifier_kind_counts") != {
        "cmsis_device_name": EXPECTED_CMSIS_ROWS,
        "ordering_pattern": EXPECTED_ORDERING_ROWS,
    }:
        raise AcquisitionError("STM32WBA5X selected identifier-kind surface drifted")
    if selection.get("next_gate") != "stm32wba5x-bounded-official-manufacturer-evidence-accessibility-gate":
        raise AcquisitionError("STM32WBA5X evidence-accessibility gate drifted")
    ...
    if len(bounded) != EXPECTED_ROWS:
        raise AcquisitionError(f"STM32WBA5X bounded row count drifted: {len(bounded)}")
    ...
    if kinds != Counter({"ordering_pattern": EXPECTED_ORDERING_ROWS, "cmsis_device_name": EXPECTED_CMSIS_ROWS}):
        raise AcquisitionError(f"STM32WBA5X identifier-kind surface drifted: {dict(kinds)}")

    subfamilies = tuple(sorted({row.get("subfamily", "") for row in bounded}))
    if subfamilies != EXPECTED_SUBFAMILIES:
        raise AcquisitionError(f"STM32WBA5X subfamily surface drifted: {subfamilies}")

    if any(row.get("mapping_status") != "mapping_candidate" for row in bounded):
        raise AcquisitionError("STM32WBA5X mapping status drifted")
    if any(row.get("validation_status") != "not_verified" for row in bounded):
        raise AcquisitionError("STM32WBA5X validation status drifted")
    if any(row.get("catalog_origin") != "openocd-parts-expanded.csv" for row in bounded):
        raise AcquisitionError("STM32WBA5X source origin drifted")
    return bounded
...
def deterministic_targets() -> list[ProbeTarget]:
    ...
    for subfamily in EXPECTED_SUBFAMILIES:
        ...
        if subfamily == "STM32WBA50":
            if bases != ["STM32WBA50KE", "STM32WBA50KG"]:
                raise AcquisitionError(f"STM32WBA50 remediation candidate set drifted: {bases}")
            base = "STM32WBA50KG"
            selection_reason = (
                "deterministic second lexical Base Device after retained "
                "STM32WBA50KE official-ST HTTP 404 blocker"
            )
        else:
            base = bases[0]
            selection_reason = (
                "deterministic lexical-min Base Device in frozen "
                "STM32WBA5X ordering-pattern subfamily"
            )
    ...
    expected_bases = ("STM32WBA50KG", "STM32WBA52CE", "STM32WBA54CE", "STM32WBA55CE", "STM32WBA5MMG")
    observed_bases = tuple(target.base_device for target in targets)
    if observed_bases != expected_bases:
        raise AcquisitionError(f"STM32WBA5X representative Base Devices drifted: {observed_bases}")
    return targets
...
def target_manifest(targets: list[ProbeTarget]) -> dict[str, object]:
    return {
        ...
        "selection_git_blob_sha": git_blob_sha(SELECTION_PATH),
        "blocked_gate_git_blob_sha": git_blob_sha(BLOCKED_GATE_PATH),
        "wireless_exhaustion_git_blob_sha": git_blob_sha(EXHAUSTION_PATH),
...
def run_live(
    ...
    *,
    ...
    summary = {
        "schema_version": 1,
        "probe_id": PROBE_ID,
        "scope": "STM32WBA5X bounded official-ST evidence remediation using deterministic alternate representative",
        ...
        "next_gate": "stm32wba5x-bounded-exact-icpn-discovery-gate" if complete else None,
        "claims": {
...
def main() -> None:
    ...
    if summary.get("bounded_probe_complete") is not True:
        raise SystemExit("STM32WBA5X evidence accessibility probe requires manual review")
