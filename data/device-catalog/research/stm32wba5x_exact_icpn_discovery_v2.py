...
15 deterministic Base Devices represented by the reconciled STM32WBA5X
...
DISCOVERY_ID = "stm32wba5x-bounded-exact-icpn-discovery-v2"
EXPECTED_ORDERING_ROWS = 15
EXPECTED_BASE_DEVICE_COUNT = 15
EXPECTED_BASES = (
    "STM32WBA50KG",
    "STM32WBA52CE",
    "STM32WBA52CG",
...
def guard_reconciliation() -> dict[str, Any]:
    ...
    if payload.get("reconciliation_id") != "stm32wba5x-current-commercial-scope-reconciliation-v2":
        raise AcquisitionError("unexpected STM32WBA5X reconciliation id")
    if payload.get("decision") != "current_commercial_scope_reconciled_v2":
    ...
    if (
        surface.get("retained_rows") != 31
        or surface.get("ordering_pattern_rows") != 15
        or surface.get("cmsis_device_name_rows") != 16
        or surface.get("base_device_count") != 15
...
def reconciled_series_rows() -> list[dict[str, str]]:
    ...
    if len(rows) != 34 or len(retained) != 31:
