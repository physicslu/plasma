#!/usr/bin/env python3
"""STM32L1 L1.4 deterministic read-only capability/admission planner."""
...
from stm32l1_phase_l1_2_discovery import DISCOVERY_ID, resolve_mapping
from stm32l1_phase_l1_3_policy import build_plan as build_metadata_plan, plan_is_clean as metadata_plan_is_clean
from validate_stm32l1_phase_l1_3_policy import main as validate_l1_3

HERE = Path(__file__).resolve().parent
PHASE = "L1.4"
POLICY_PHASE = "L1.3"
DISCOVERY_PHASE = "L1.2"
PUBLICATION_PHASE = "L1.5"
ADAPTER_ID = "stm32l1-l1.4-admission"
METADATA_ADAPTER_ID = "stm32l1-l1.3-metadata"
DEFAULT_CANONICAL = HERE / "stm32l1-commercial-icpn.csv"
...
EXPECTED_POLICY_READY_EXACT_SET_SHA256 = "0fcc20ca062da9c7e564b141b38c529c2c3a27a6ca781a2473f9f617e2886a12"
...
EXPECTED_PRODUCTION_MANIFEST_GIT_BLOB = "1aa2311a25a69742c428147a402816ed5071e04e"
EXPECTED_PRODUCTION_MANIFEST_SHA256 = "b88adcb38f0833a25da1671592ea98496d61166b9d0e51877ffb92ad2820b9fe"


class STM32L1AdmissionError(AdmissionError):
    pass
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise STM32L1AdmissionError(f"{path.name}: expected JSON object")
    return value
...
def _canonical_prestate(canonical_path: Path | None) -> tuple[list[str], list[dict[str, str]], bool]:
    ...
    if tuple(fields) != CANONICAL_FIELDS:
        raise STM32L1AdmissionError("STM32L1 canonical CSV schema mismatch")
    if any(row.get("family") not in ("", FAMILY) for row in rows):
        raise STM32L1AdmissionError("STM32L1 canonical CSV contains a foreign family")
    identities = [row.get("icpn", "") for row in rows]
    if len(identities) != len(set(identities)):
        raise STM32L1AdmissionError("STM32L1 canonical CSV contains duplicate ICPNs")
    if rows:
        raise STM32L1AdmissionError(f"L1.4 requires zero-row STM32L1 canonical prestate, got {len(rows)}")
...
def build_admission_plan(
    *,
    ...
    if validate_l1_3() != 0:
        raise STM32L1AdmissionError("L1.3 hard-lock validation failed")
    metadata_plan = build_metadata_plan()
    if not metadata_plan_is_clean(metadata_plan):
        raise STM32L1AdmissionError("L1.3 metadata plan is not clean")

    baseline = _read_json(DEFAULT_POLICY_BASELINE)
    if _git_blob_sha(DEFAULT_POLICY_BASELINE) != EXPECTED_METADATA_BASELINE_GIT_BLOB:
        raise STM32L1AdmissionError("L1.3 metadata baseline bytes drifted")
    if _sha256(DEFAULT_POLICY_BASELINE) != EXPECTED_METADATA_BASELINE_SHA256:
        raise STM32L1AdmissionError("L1.3 metadata baseline digest drifted")
    result = baseline.get("result")
    if not isinstance(result, dict):
        raise STM32L1AdmissionError("L1.3 metadata baseline result missing")
    if result.get("metadata_rows_sha256") != EXPECTED_METADATA_ROWS_SHA256:
        raise STM32L1AdmissionError("L1.3 metadata rows digest drifted")
    if result.get("metadata_ready_set_sha256") != EXPECTED_POLICY_READY_EXACT_SET_SHA256:
        raise STM32L1AdmissionError("L1.3 metadata-ready exact set drifted")
    if result.get("decision_counts") != {"metadata_ready": 144, "manual_review_required": 0, "reject": 0}:
        raise STM32L1AdmissionError("L1.3 metadata disposition drifted")
    ...
    if len(all_candidates) != EXPECTED_ACTIVE_EXACT_COUNT:
        raise STM32L1AdmissionError("L1.3 exact candidate count drifted")
    ...
    for source_candidate in all_candidates:
        ...
        if not isinstance(icpn, str):
            raise STM32L1AdmissionError("candidate lacks exact ICPN")
    ...
    if sum(routing.values()) != EXPECTED_ACTIVE_EXACT_COUNT:
        raise STM32L1AdmissionError("L1.4 routing replay cardinality drifted")
    if set(routing) - {"unique", "ambiguous", "unmapped"}:
        raise STM32L1AdmissionError(f"L1.4 unsupported routing states: {dict(routing)}")
    ...
    plan = build_framework_plan(
        ...
        input_bindings={
            ...
            "policy_baseline_sha256": EXPECTED_METADATA_BASELINE_SHA256,
            ...
            "mapping_catalog_sha256": EXPECTED_OPENOCD_SHA256,
    ...
    plan.update({
        ...
        "manufacturer_verified_identity_count": EXPECTED_ACTIVE_EXACT_COUNT,
        "metadata_ready_count": EXPECTED_ACTIVE_EXACT_COUNT,
        ...
        "full_stm32l1_surface_covered": False,
        "fail_closed": True,
    })
    return plan
...
def admission_plan_is_clean(plan: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and plan.get("manufacturer_verified_identity_count") == EXPECTED_ACTIVE_EXACT_COUNT
        and plan.get("metadata_ready_count") == EXPECTED_ACTIVE_EXACT_COUNT
        and plan.get("candidate_count") == plan.get("capability_admittable_count")
        and plan.get("capability_admittable_count", 0) + plan.get("capability_unresolved_count", 0) == EXPECTED_ACTIVE_EXACT_COUNT
        and sum(replay.get(key, 0) for key in ("unique", "ambiguous", "unmapped")) == EXPECTED_ACTIVE_EXACT_COUNT
        ...
        and plan.get("full_stm32l1_surface_covered") is False
...
def main() -> int:
    ...
    value = admission_summary(plan) if args.summary_only else plan
