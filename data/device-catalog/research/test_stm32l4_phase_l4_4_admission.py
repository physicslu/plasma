...
from stm32l4_admission_policy import TARGET_CONFIG
from stm32l4_phase_l4_2_discovery import commercial_core
from stm32l4_phase_l4_4_admission import admission_plan_is_clean, build_admission_plan


class STM32L4PhaseL44AdmissionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.plan = build_admission_plan()

    def test_all_446_metadata_ready_identities_are_uniquely_routable(self) -> None:
        self.assertEqual(self.plan["manufacturer_verified_identity_count"], 446)
        self.assertEqual(self.plan["metadata_ready_count"], 446)
        self.assertEqual(self.plan["capability_admittable_count"], 446)
        ...
        self.assertEqual(
            ...
            {"unique": 446, "ambiguous": 0, "unmapped": 0},
        )

    def test_admitted_candidates_have_one_l4_ordering_pattern_route(self) -> None:
        self.assertEqual(self.plan["candidate_count"], 446)
    ...
    def test_framework_plan_is_clean_for_all_446_candidates(self) -> None:
        self.assertEqual(
            self.plan["decision_counts"],
            {"admit": 446, "already_present": 0, "manual_review_required": 0, "reject": 0},
        )
    ...
    def test_l43_exact_variant_exceptions_remain_exact_identity_only(self) -> None:
        expected = {"STM32L4A6RGT7", "STM32L4A6RGT7TR", "STM32L4S5QII3P"}
        observed = {item["icpn"] for item in self.plan["candidates"] if item["icpn"] in expected}
        self.assertEqual(observed, expected)
        for item in self.plan["candidates"]:
            if item["icpn"] in expected:
                self.assertEqual(item["decision"], "admit")
                self.assertEqual(item["base_mapping"]["status"], "unique")
    ...
    def test_production_and_runtime_claims_remain_unchanged(self) -> None:
        ...
        self.assertEqual(production["family_count"], 11)
        self.assertEqual(production["stm32l4_exact_icpn_count"], 0)
