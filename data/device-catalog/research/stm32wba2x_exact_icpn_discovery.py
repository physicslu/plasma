#!/usr/bin/env python3
"""Bounded official-ST exact ICPN discovery for STM32WBA2X.

This research-only transaction enumerates exact commercial identities from the
four deterministic Base Devices represented by the frozen STM32WBA2X
...
ACCESSIBILITY = HERE / "evidence/stm32wba2x-accessibility-live-2026-09-18/probe-summary.json"

DISCOVERY_ID = "stm32wba2x-bounded-exact-icpn-discovery-v1"
EXPECTED_ORDERING_ROWS = 4
EXPECTED_BASE_DEVICE_COUNT = 4
EXPECTED_BASES = (
    "STM32WBA23CE",
    "STM32WBA23KE",
    "STM32WBA25CE",
...
NEXT_GATE = "stm32wba2x-bounded-exact-icpn-admission-readiness-gate"
...
def guard_accessibility() -> dict[str, Any]:
    ...
    if payload.get("probe_id") != "stm32wba2x-bounded-official-st-evidence-accessibility-v1":
        raise AcquisitionError("unexpected STM32WBA2X accessibility probe id")
    if payload.get("selected_wireless_frontier") != EXPECTED_SERIES:
        raise AcquisitionError("STM32WBA2X accessibility frontier drifted")
    if payload.get("target_config") != EXPECTED_TARGET_CONFIG:
        raise AcquisitionError("STM32WBA2X accessibility target config drifted")
    if payload.get("attempted_targets") != 2 or payload.get("successful_targets") != 2:
        raise AcquisitionError("STM32WBA2X accessibility coverage drifted")
    if payload.get("manual_review_targets") != 0:
        raise AcquisitionError("STM32WBA2X accessibility retains manual review")
    if payload.get("official_st_evidence_accessible_for_all_subfamilies") is not True:
        raise AcquisitionError("STM32WBA2X official-ST accessibility gate is incomplete")
    if payload.get("next_gate") != "stm32wba2x-bounded-exact-icpn-discovery-gate":
        raise AcquisitionError("STM32WBA2X accessibility next gate drifted")
    claims = payload.get("claims")
    if not isinstance(claims, dict) or any(value is not False for value in claims.values()):
        raise AcquisitionError("STM32WBA2X accessibility claims escaped fail-closed state")
    return payload
...
def deterministic_targets() -> list[ProbeTarget]:
    ...
    if len(ordering_rows) != EXPECTED_ORDERING_ROWS:
        raise AcquisitionError(f"STM32WBA2X ordering-pattern row count drifted: {len(ordering_rows)}")
        ...
        for base in bases:
            ...
            targets.append(
                ProbeTarget(
                    ...
                    base_device=base,
                    source_url=url,
                    selection_reason="unique Base Device derived from frozen STM32WBA2X ordering-pattern rows",
                )
            )

    observed = tuple(target.base_device for target in targets)
    if observed != EXPECTED_BASES:
        raise AcquisitionError(f"STM32WBA2X deterministic Base Devices drifted: {observed}")
    if len({target.base_device for target in targets}) != EXPECTED_BASE_DEVICE_COUNT:
        raise AcquisitionError("STM32WBA2X deterministic targets are not unique")
    if {target.subfamily for target in targets} != set(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("STM32WBA2X target subfamily coverage drifted")
    return targets
...
def run_live(
    ...
    *,
    ...
    if targets != deterministic_targets():
        raise AcquisitionError("STM32WBA2X discovery target set drifted")
    ...
    if complete and not exact_list:
        raise AcquisitionError("STM32WBA2X complete discovery yielded no Active exact ICPNs")
    ...
    return {
        "schema_version": 1,
        "discovery_id": DISCOVERY_ID,
        "scope": "STM32WBA2X bounded official-ST exact ICPN discovery",
        "selected_wireless_frontier": EXPECTED_SERIES,
        "target_config": EXPECTED_TARGET_CONFIG,
        "base_device_count": len(targets),
...
def main() -> None:
    ...
    if summary.get("bounded_exact_discovery_complete") is not True:
        raise SystemExit("STM32WBA2X exact ICPN discovery requires manual review")
