#!/usr/bin/env python3
"""Retain a clean STM32G0 Phase 4.8B live artifact without re-acquisition."""
...
DISCOVERY_MANIFEST=HERE/'stm32g0-phase4.8b-discovery-manifest.json'
...
EVIDENCE_DIR=HERE/'evidence/stm32g0-phase4.8b-official-st-discovery-live-2026-09-09'
PHASE='4.8B'; FAMILY='STM32G0'; PROFILE='stm32g0_dual_surface_v1'; MAX_TARGETS=12
...
def source_bindings()->dict[str,object]:
    ...
    req(sum(counts.values())==563,f'Production exact count drifted: {sum(counts.values())}')
    req(counts.get('STM32F7')==19,'F7 publication prestate missing')
    req('STM32G0' not in counts,'G0 must be absent from Production in 4.8B')
    return {'discovery_manifest_git_blob_sha':blob(DISCOVERY_MANIFEST),'openocd_catalog_git_blob_sha':blob(OPENOCD),
            'production_manifest_git_blob_sha':blob(PRODUCTION),'production_exact_icpn_count':563,
            'production_family_counts':counts,'stm32g0_production_prestate_count':0}
...
def retain(*,artifact_root:Path,artifact_id:int,artifact_zip_sha256:str,workflow_run_id:int,executed_git_sha:str)->dict[str,object]:
    ...
    summary_path=artifact_root/'stm32g0-phase4.8b-live-summary.json'
    rawdir=artifact_root/'stm32g0-4.8b-evidence'
    ...
    evidence_id=f"stm32g0-phase4.8b-official-st-discovery-2026-09-09-retained-{times[-1].replace('-','').replace(':','')}-{executed_git_sha[:8]}"
    ...
    readme=f"# STM32G0 Phase 4.8B retained evidence\n\nCompact projection of immutable GitHub Actions artifact {artifact_id} from run {workflow_run_id}. Raw browser evidence remains in the artifact. No Production or runtime-support claim is authorized.\n"
