...
class STM32U0PhaseU01FoundationTests(unittest.TestCase):
    @classmethod
    ...
    def test_identifier_kind_drift_fails_closed(self) -> None:
        ...
        victim = next(
            row
            for row in rows
            if row.get("plasma_series") == "STM32U0"
    ...
    def test_target_config_drift_fails_closed(self) -> None:
        rows = copy.deepcopy(self.rows)
        victim = next(row for row in rows if row.get("plasma_series") == "STM32U0")
    ...
    def test_malformed_ordering_pattern_fails_closed(self) -> None:
        ...
        victim["part_number"] = "STM32U0_NOT_AN_ORDERING_PATTERN"
        with self.assertRaises(AcquisitionError):
            build_foundation_report(rows)
    ...
    def test_selection_is_hard_bound_and_research_only(self) -> None:
        validate_selection()
        payload_path = HERE / "stm32-next-family-selection.json"
    ...
    def test_retained_identity_is_hard_bound_and_exact(self) -> None:
        targets = validate_retained_identity()
        self.assertEqual(set(targets), {"STM32U031C6", "STM32U073C8", "STM32U083CC"})
        self.assertEqual(sum(len(target["exact_icpns"]) for target in targets.values()), 8)

    def test_ordering_authority_is_hard_bound_and_official_st(self) -> None:
        targets = validate_ordering_authority()
        self.assertEqual(set(targets), {"STM32U031C6", "STM32U073C8", "STM32U083CC"})
        self.assertEqual(
            {target["datasheet_id"] for target in targets.values()},
            {"DS14581", "DS14548", "DS14463"},
        )
