#!/usr/bin/env python3
"""Build the bounded Phase 4.3D STM32F2 admission plan without writing Production."""
...
from stm32f2_admission_policy import CANONICAL_FIELDS, FAMILY, build_canonical_row
from stm32f2_phase4_3c_policy import (
...
def build_admission_plan(
    *,
    ...
    if not policy_plan_is_clean(policy_plan) or policy_summary(policy_plan) != policy_baseline:
        raise AdmissionError("Phase 4.3C policy is not closed at its guarded boundary")
    ...
    plan = build_framework_plan(
        ...
        input_bindings={
            ...
            "retained_evidence_baseline": DEFAULT_BASELINE.name,
            "retained_evidence_baseline_sha256": file_sha256(DEFAULT_BASELINE),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not admission_plan_is_clean(plan):
            raise AdmissionError("Phase 4.3D admission plan is not clean")
