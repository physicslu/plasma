#!/usr/bin/env python3
"""Bounded official-ST evidence accessibility probe for STM32WLX.

This transaction proves only that one deterministic representative Base Device
per frozen STM32WLX subfamily has accessible official ST commercial identity
...
The frozen STM32WL55 cohort contains one ordering pattern (STM32WL5MOCHx)
whose derived Base Device is not prefix-consistent with the STM32WL55
subfamily label. This probe retains that anomaly as an explicit guarded input
but does not use it as the bounded representative.
"""
...
SELECTION_PATH = HERE / "stm32-post-wba2x-wireless-frontier-selection.json"

PROBE_ID = "stm32wlx-bounded-official-st-evidence-accessibility-v1"
EXPECTED_SOURCE_SHA256 = "43ca9f9bbd2826aef8cd147a251263255d957dd1bcac7da653905d3bf980b6a3"
EXPECTED_TARGET_CONFIG = "tcl/target/stm32wlx.cfg"
EXPECTED_FAMILY = "STM32WL Series"
EXPECTED_SERIES = "STM32WLX"
EXPECTED_SUBFAMILIES = ("STM32WL54", "STM32WL55", "STM32WLE4", "STM32WLE5")
EXPECTED_ROWS = 17
EXPECTED_ORDERING_ROWS = 17
EXPECTED_CMSIS_ROWS = 0
EXPECTED_NON_PREFIX_PATTERN = "STM32WL5MOCHx"
EXPECTED_NON_PREFIX_BASE = "STM32WL5MOC"
EXPECTED_BASES = ("STM32WL54CC", "STM32WL55CC", "STM32WLE4C8", "STM32WLE5C8")
...
def ordering_base(row: dict[str, str]) -> str:
...
def guarded_series_rows() -> list[dict[str, str]]:
    ...
    if selection.get("selection_id") != "stm32-post-wba2x-wireless-frontier-selection-v1":
        raise AcquisitionError("unexpected post-WBA2X wireless frontier selection id")
    if selection.get("selected_wireless_frontier") != EXPECTED_SERIES:
        raise AcquisitionError("STM32WLX is no longer the selected wireless frontier")
    if selection.get("selected_target_config") != EXPECTED_TARGET_CONFIG:
        raise AcquisitionError("STM32WLX selected target config drifted")
    if selection.get("selected_row_count") != EXPECTED_ROWS:
        raise AcquisitionError("STM32WLX selected row count drifted")
    if selection.get("selected_subfamily_count") != len(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("STM32WLX selected subfamily count drifted")
    if tuple(selection.get("selected_subfamilies") or ()) != EXPECTED_SUBFAMILIES:
        raise AcquisitionError("STM32WLX selected subfamilies drifted")
    if selection.get("selected_identifier_kind_counts") != {
        "ordering_pattern": EXPECTED_ORDERING_ROWS,
    }:
        raise AcquisitionError("STM32WLX selected identifier-kind surface drifted")
    if selection.get("next_gate") != "stm32wlx-bounded-official-manufacturer-evidence-accessibility-gate":
        raise AcquisitionError("STM32WLX evidence-accessibility gate drifted")
    ...
    if len(bounded) != EXPECTED_ROWS:
        raise AcquisitionError(f"STM32WLX bounded row count drifted: {len(bounded)}")

    kinds = Counter(row.get("identifier_kind", "") for row in bounded)
    if kinds != Counter({"ordering_pattern": EXPECTED_ORDERING_ROWS}):
        raise AcquisitionError(f"STM32WLX identifier-kind surface drifted: {dict(kinds)}")

    subfamilies = tuple(sorted({row.get("subfamily", "") for row in bounded}))
    if subfamilies != EXPECTED_SUBFAMILIES:
        raise AcquisitionError(f"STM32WLX subfamily surface drifted: {subfamilies}")

    if any(row.get("mapping_status") != "mapping_candidate" for row in bounded):
        raise AcquisitionError("STM32WLX mapping status drifted")
    if any(row.get("validation_status") != "not_verified" for row in bounded):
        raise AcquisitionError("STM32WLX validation status drifted")
    if any(row.get("catalog_origin") != "openocd-parts-expanded.csv" for row in bounded):
        raise AcquisitionError("STM32WLX source origin drifted")

    non_prefix = []
    for row in bounded:
        base = ordering_base(row)
        if not base.startswith(row["subfamily"]):
            non_prefix.append((row["subfamily"], row["part_number"], base))
    if non_prefix != [("STM32WL55", EXPECTED_NON_PREFIX_PATTERN, EXPECTED_NON_PREFIX_BASE)]:
        raise AcquisitionError(f"STM32WLX non-prefix ordering anomaly drifted: {non_prefix}")
    return bounded
...
def deterministic_targets() -> list[ProbeTarget]:
    ...
    for row in rows:
        base = ordering_base(row)
        if base.startswith(row["subfamily"]):
    ...
    for subfamily in EXPECTED_SUBFAMILIES:
        ...
        if not bases:
            raise AcquisitionError(f"{subfamily}: no prefix-consistent ordering-pattern Base Device")
        base = bases[0]
        ...
        targets.append(
            ProbeTarget(
                ...
                source_url=url,
                selection_reason=(
                    "deterministic lexical-min prefix-consistent Base Device in frozen "
                    "STM32WLX ordering-pattern subfamily"
                ),
            )
        )
    ...
    if observed_bases != EXPECTED_BASES:
        raise AcquisitionError(f"STM32WLX representative Base Devices drifted: {observed_bases}")
    return targets
...
def target_manifest(targets: list[ProbeTarget]) -> dict[str, object]:
    return {
        ...
        "known_non_prefix_ordering_rows": [
            {
                "subfamily": "STM32WL55",
                "part_number": EXPECTED_NON_PREFIX_PATTERN,
                "derived_base_device": EXPECTED_NON_PREFIX_BASE,
...
def run_live(
    ...
    *,
    ...
    summary = {
        "schema_version": 1,
        "probe_id": PROBE_ID,
        "scope": "STM32WLX bounded representative official-ST commercial identity/lifecycle evidence accessibility",
        ...
        "next_gate": "stm32wlx-bounded-exact-icpn-discovery-gate" if complete else None,
        "claims": {
...
def main() -> None:
    ...
    if summary.get("bounded_probe_complete") is not True:
        raise SystemExit("STM32WLX evidence accessibility probe requires manual review")
