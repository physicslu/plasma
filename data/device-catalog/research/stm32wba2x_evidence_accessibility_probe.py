#!/usr/bin/env python3
"""Bounded official-ST evidence accessibility probe for STM32WBA2X.

This transaction proves only that one deterministic representative Base Device
per frozen STM32WBA2X subfamily has accessible official ST commercial identity
...
SELECTION_PATH = HERE / "stm32-post-h7-wireless-frontier-selection.json"

PROBE_ID = "stm32wba2x-bounded-official-st-evidence-accessibility-v1"
EXPECTED_SOURCE_SHA256 = "43ca9f9bbd2826aef8cd147a251263255d957dd1bcac7da653905d3bf980b6a3"
EXPECTED_TARGET_CONFIG = "tcl/target/stm32wba2x.cfg"
EXPECTED_FAMILY = "STM32WBA Series"
EXPECTED_SERIES = "STM32WBA2X"
EXPECTED_SUBFAMILIES = ("STM32WBA23", "STM32WBA25")
EXPECTED_ROWS = 8
EXPECTED_ORDERING_ROWS = 4
...
def guarded_series_rows() -> list[dict[str, str]]:
    ...
    if selection.get("selection_id") != "stm32-post-h7-wireless-frontier-selection-v1":
        raise AcquisitionError("unexpected post-H7 wireless frontier selection id")
    if selection.get("selected_wireless_frontier") != EXPECTED_SERIES:
        raise AcquisitionError("STM32WBA2X is no longer the selected wireless frontier")
    if selection.get("selected_target_config") != EXPECTED_TARGET_CONFIG:
        raise AcquisitionError("STM32WBA2X selected target config drifted")
    if selection.get("selected_row_count") != EXPECTED_ROWS:
        raise AcquisitionError("STM32WBA2X selected row count drifted")
    if selection.get("selected_subfamily_count") != len(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("STM32WBA2X selected subfamily count drifted")
    if tuple(selection.get("selected_subfamilies") or ()) != EXPECTED_SUBFAMILIES:
        raise AcquisitionError("STM32WBA2X selected subfamilies drifted")
    if selection.get("selected_identifier_kind_counts") != {
        ...
        raise AcquisitionError("STM32WBA2X selected identifier-kind surface drifted")
    if selection.get("next_gate") != "stm32wba2x-bounded-official-manufacturer-evidence-accessibility-gate":
        raise AcquisitionError("STM32WBA2X evidence-accessibility gate drifted")
    ...
    if len(bounded) != EXPECTED_ROWS:
        raise AcquisitionError(f"STM32WBA2X bounded row count drifted: {len(bounded)}")
    ...
    if kinds != Counter({"ordering_pattern": EXPECTED_ORDERING_ROWS, "cmsis_device_name": EXPECTED_CMSIS_ROWS}):
        raise AcquisitionError(f"STM32WBA2X identifier-kind surface drifted: {dict(kinds)}")

    subfamilies = tuple(sorted({row.get("subfamily", "") for row in bounded}))
    if subfamilies != EXPECTED_SUBFAMILIES:
        raise AcquisitionError(f"STM32WBA2X subfamily surface drifted: {subfamilies}")

    if any(row.get("mapping_status") != "mapping_candidate" for row in bounded):
        raise AcquisitionError("STM32WBA2X mapping status drifted")
    if any(row.get("validation_status") != "not_verified" for row in bounded):
        raise AcquisitionError("STM32WBA2X validation status drifted")
    if any(row.get("catalog_origin") != "openocd-parts-expanded.csv" for row in bounded):
        raise AcquisitionError("STM32WBA2X source origin drifted")
    return bounded
...
def deterministic_targets() -> list[ProbeTarget]:
        ...
        targets.append(
            ProbeTarget(
                ...
                selection_reason=(
                    ...
                    "STM32WBA2X ordering-pattern subfamily"
                ),
            )
        )
    ...
    if observed_bases != expected_bases:
        raise AcquisitionError(f"STM32WBA2X representative Base Devices drifted: {observed_bases}")
    return targets
...
def run_live(
    ...
    *,
    ...
    summary = {
        "schema_version": 1,
        "probe_id": PROBE_ID,
        "scope": "STM32WBA2X bounded representative official-ST commercial identity/lifecycle evidence accessibility",
        ...
        "next_gate": "stm32wba2x-bounded-exact-icpn-discovery-gate" if complete else None,
        "claims": {
...
def main() -> None:
    ...
    if summary.get("bounded_probe_complete") is not True:
        raise SystemExit("STM32WBA2X evidence accessibility probe requires manual review")
