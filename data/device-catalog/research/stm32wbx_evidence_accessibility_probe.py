#!/usr/bin/env python3
"""Bounded official-ST evidence accessibility probe for STM32WBX.

This transaction proves only that one deterministic representative Base Device
per frozen STM32WBX subfamily has accessible official ST commercial identity
...
SELECTION_PATH = HERE / "stm32-post-wba6x-blocked-wireless-frontier-selection.json"

PROBE_ID = "stm32wbx-bounded-official-st-evidence-accessibility-v1"
EXPECTED_SOURCE_SHA256 = "43ca9f9bbd2826aef8cd147a251263255d957dd1bcac7da653905d3bf980b6a3"
EXPECTED_TARGET_CONFIG = "tcl/target/stm32wbx.cfg"
EXPECTED_FAMILY = "STM32WB Series"
EXPECTED_SERIES = "STM32WBX"
EXPECTED_SUBFAMILIES = ("STM32WB10", "STM32WB15", "STM32WB1M", "STM32WB30", "STM32WB35", "STM32WB50", "STM32WB55", "STM32WB5M")
EXPECTED_ROWS = 23
EXPECTED_ORDERING_ROWS = 19
EXPECTED_CMSIS_ROWS = 4
...
def guarded_series_rows() -> list[dict[str, str]]:
    ...
    if selection.get("selection_id") != "stm32-post-wba6x-evidence-block-wireless-frontier-selection-v1":
        raise AcquisitionError("unexpected post-WBA6X-block wireless frontier selection id")
    if selection.get("selected_wireless_frontier") != EXPECTED_SERIES:
        raise AcquisitionError("STM32WBX is no longer the selected wireless frontier")
    if selection.get("selected_target_config") != EXPECTED_TARGET_CONFIG:
        raise AcquisitionError("STM32WBX selected target config drifted")
    if selection.get("selected_row_count") != EXPECTED_ROWS:
        raise AcquisitionError("STM32WBX selected row count drifted")
    if selection.get("selected_subfamily_count") != len(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("STM32WBX selected subfamily count drifted")
    if tuple(selection.get("selected_subfamilies") or ()) != EXPECTED_SUBFAMILIES:
        raise AcquisitionError("STM32WBX selected subfamilies drifted")
    if selection.get("selected_identifier_kind_counts") != {
        ...
        raise AcquisitionError("STM32WBX selected identifier-kind surface drifted")
    if selection.get("next_gate") != "stm32wbx-bounded-official-manufacturer-evidence-accessibility-gate":
        raise AcquisitionError("STM32WBX evidence-accessibility gate drifted")
    ...
    if len(bounded) != EXPECTED_ROWS:
        raise AcquisitionError(f"STM32WBX bounded row count drifted: {len(bounded)}")
    ...
    if kinds != Counter({"ordering_pattern": EXPECTED_ORDERING_ROWS, "cmsis_device_name": EXPECTED_CMSIS_ROWS}):
        raise AcquisitionError(f"STM32WBX identifier-kind surface drifted: {dict(kinds)}")

    subfamilies = tuple(sorted({row.get("subfamily", "") for row in bounded}))
    if subfamilies != EXPECTED_SUBFAMILIES:
        raise AcquisitionError(f"STM32WBX subfamily surface drifted: {subfamilies}")

    if any(row.get("mapping_status") != "mapping_candidate" for row in bounded):
        raise AcquisitionError("STM32WBX mapping status drifted")
    if any(row.get("validation_status") != "not_verified" for row in bounded):
        raise AcquisitionError("STM32WBX validation status drifted")
    if any(row.get("catalog_origin") != "openocd-parts-expanded.csv" for row in bounded):
        raise AcquisitionError("STM32WBX source origin drifted")
    return bounded
...
def deterministic_targets() -> list[ProbeTarget]:
    ...
    base_kinds: dict[tuple[str, str], set[str]] = defaultdict(set)
    for row in rows:
        ...
        if kind == "ordering_pattern":
            ...
            raise AcquisitionError(f"unsupported STM32WBX identifier kind: {kind!r}")
        subfamily = row["subfamily"]
        by_subfamily[subfamily].add(base)
        base_kinds[(subfamily, base)].add(kind)

    targets: list[ProbeTarget] = []
    for subfamily in EXPECTED_SUBFAMILIES:
        bases = sorted(by_subfamily[subfamily])
        if not bases:
            raise AcquisitionError(f"{subfamily}: no commercial Base Device")
        base = bases[0]
        url = source_url_for_base(base)
        validate_source_url(url)
        if not base.startswith(subfamily):
            raise AcquisitionError(f"{base}: escaped {subfamily} boundary")
        kinds = sorted(base_kinds[(subfamily, base)])
        targets.append(
            ProbeTarget(
                ...
                source_url=url,
                selection_reason=(
                    "deterministic lexical-min Base Device from frozen STM32WBX "
                    f"commercial identifier surface ({'+'.join(kinds)})"
                ),
            )
        )
    ...
    if observed_bases != expected_bases:
        raise AcquisitionError(f"STM32WBX representative Base Devices drifted: {observed_bases}")
    return targets
...
def run_live(
    ...
    *,
    ...
    summary = {
        "schema_version": 1,
        "probe_id": PROBE_ID,
        "scope": "STM32WBX bounded representative official-ST commercial identity/lifecycle evidence accessibility",
        ...
        "next_gate": "stm32wbx-bounded-exact-icpn-discovery-gate" if complete else None,
        "claims": {
...
def main() -> None:
    ...
    if summary.get("bounded_probe_complete") is not True:
        raise SystemExit("STM32WBX evidence accessibility probe requires manual review")
