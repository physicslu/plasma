#!/usr/bin/env python3
"""Bounded official-ST exact ICPN discovery for STM32WBX.

This research-only transaction enumerates exact commercial identities from the
four deterministic Base Devices represented by the frozen STM32WBX
...
from stm32wbx_evidence_accessibility_probe import (
    ...
    SELECTION_PATH,
    SOURCE_PATH,
    base_from_cmsis_identifier,
    guarded_series_rows,
)

HERE = Path(__file__).resolve().parent
ACCESSIBILITY = HERE / "evidence/stm32wbx-accessibility-live-2026-09-22/probe-summary.json"

DISCOVERY_ID = "stm32wbx-bounded-exact-icpn-discovery-v1"
...
NEXT_GATE = "stm32wbx-bounded-exact-icpn-admission-readiness-gate"
...
def guard_accessibility() -> dict[str, Any]:
    ...
    if payload.get("probe_id") != "stm32wbx-bounded-official-st-evidence-accessibility-v1":
        raise AcquisitionError("unexpected STM32WBX accessibility probe id")
    if payload.get("selected_wireless_frontier") != EXPECTED_SERIES:
        raise AcquisitionError("STM32WBX accessibility frontier drifted")
    if payload.get("target_config") != EXPECTED_TARGET_CONFIG:
        raise AcquisitionError("STM32WBX accessibility target config drifted")
    if payload.get("attempted_targets") != 8 or payload.get("successful_targets") != 8:
        raise AcquisitionError("STM32WBX accessibility coverage drifted")
    if payload.get("manual_review_targets") != 0:
        raise AcquisitionError("STM32WBX accessibility retains manual review")
    if payload.get("official_st_evidence_accessible_for_all_subfamilies") is not True:
        raise AcquisitionError("STM32WBX official-ST accessibility gate is incomplete")
    if payload.get("next_gate") != "stm32wbx-bounded-exact-icpn-discovery-gate":
        raise AcquisitionError("STM32WBX accessibility next gate drifted")
    claims = payload.get("claims")
    if not isinstance(claims, dict) or any(value is not False for value in claims.values()):
        raise AcquisitionError("STM32WBX accessibility claims escaped fail-closed state")
    return payload
...
def deterministic_targets() -> list[ProbeTarget]:
    ...
    if len(ordering_rows) != EXPECTED_ORDERING_ROWS:
        raise AcquisitionError(f"STM32WBX ordering-pattern row count drifted: {len(ordering_rows)}")
    if len(cmsis_rows) != 4:
        raise AcquisitionError(f"STM32WBX CMSIS row count drifted: {len(cmsis_rows)}")
        ...
        for base in bases:
            ...
            targets.append(
                ProbeTarget(
                    ...
                    base_device=base,
                    source_url=url,
                    selection_reason="unique commercial Base Device derived from frozen STM32WBX identifier rows",
                )
            )

    observed = tuple(target.base_device for target in targets)
    if observed != EXPECTED_BASES:
        raise AcquisitionError(f"STM32WBX deterministic Base Devices drifted: {observed}")
    if len({target.base_device for target in targets}) != EXPECTED_BASE_DEVICE_COUNT:
        raise AcquisitionError("STM32WBX deterministic targets are not unique")
    if {target.subfamily for target in targets} != set(EXPECTED_SUBFAMILIES):
        raise AcquisitionError("STM32WBX target subfamily coverage drifted")
    return targets
...
def run_live(
    ...
    *,
    ...
    if targets != deterministic_targets():
        raise AcquisitionError("STM32WBX discovery target set drifted")
    ...
    if complete and not exact_list:
        raise AcquisitionError("STM32WBX complete discovery yielded no Active exact ICPNs")
    ...
    return {
        "schema_version": 1,
        "discovery_id": DISCOVERY_ID,
        "scope": "STM32WBX bounded official-ST exact ICPN discovery",
        "selected_wireless_frontier": EXPECTED_SERIES,
        "target_config": EXPECTED_TARGET_CONFIG,
        "base_device_count": len(targets),
...
def main() -> None:
    ...
    if summary.get("bounded_exact_discovery_complete") is not True:
        raise SystemExit("STM32WBX exact ICPN discovery requires manual review")
