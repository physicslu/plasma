#!/usr/bin/env python3
"""Validate immutable Phase 4.3E STM32F2 official-ST discovery evidence."""
...
DEFAULT_BASELINE = HERE / "stm32f2-phase4.3e-discovery-baseline.json"
DEFAULT_EVIDENCE_DIR = (
    HERE / "evidence" / "stm32f2-phase4.3e-official-st-discovery-live-2026-09-07"
)
...
def validate(*, evidence_dir: Path, baseline_path: Path) -> dict[str, Any]:
    ...
    require(baseline.get("phase") == "4.3E", "baseline phase mismatch")
    ...
    require(summary.get("phase") == "4.3E", "retained summary phase mismatch")
    ...
    require(discovery_is_clean(summary), "retained Phase 4.3E discovery is not clean")
    ...
    require(core["exact_icpn_candidate_count"] == 13, "provenance candidate count mismatch")
    ...
    require(mapping_count == 13, "retained exact ICPN mapping count mismatch")
    ...
    return {
        ...
        "active_exact_icpn_candidates": 13,
        "excluded_non_active_part_numbers": 0,
        "unique_openocd_mappings": 13,
