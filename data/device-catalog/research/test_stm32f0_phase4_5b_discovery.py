...
from stm32f0_foundation import DEFAULT_CATALOG, deterministic_initial_targets, read_catalog
...
class STM32F0Phase45BDiscoveryTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def evidence_builder(**kwargs):
        ...
        return {
            "base_device": base,
            "parser_profile": "stm32f0_dual_surface_v1",
    ...
    def test_manifest_exactly_replays_phase45a_selection(self) -> None:
    ...
    def test_clean_identity_discovery_is_not_gated_by_unmapped_openocd(self) -> None:
        with patch(
            "stm32f0_phase4_5b_discovery.resolve_mapping",
    ...
    def test_manifest_target_drift_fails_closed(self) -> None:
        ...
        payload["targets"][0]["base_device"] = "STM32F030F4"
        payload["targets"][0]["source_url"] = source_url_for_base("STM32F030F4")
    ...
    def test_manifest_url_slug_drift_fails_closed(self) -> None:
        payload = json.loads(DEFAULT_MANIFEST.read_text(encoding="utf-8"))
        payload["targets"][0]["source_url"] = source_url_for_base("STM32F031C4")
        with tempfile.TemporaryDirectory() as root:
            path = Path(root) / "manifest.json"
            path.write_text(json.dumps(payload), encoding="utf-8")
