...
def test_current_production_metrics() -> None:
    ...
    assert metrics["exact_icpns"] == 340
    assert metrics["families"] == 2
    assert metrics["family_exact_icpns"] == {"STM32F1": 75, "STM32F4": 265}
    assert metrics["base_devices"] == 109
    assert metrics["deterministic_openocd_exact_icpns"] == 340
    assert metrics["ic_support_bound_exact_icpns"] == 2
    assert metrics["unresolved_programming_profile_exact_icpns"] == 338
