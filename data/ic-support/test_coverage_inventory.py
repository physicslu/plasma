...
def test_current_production_metrics() -> None:
    ...
    assert metrics["exact_icpns"] == 468
    assert metrics["families"] == 3
    assert metrics["family_exact_icpns"] == {"STM32F1": 75, "STM32F2": 9, "STM32F4": 384}
    assert metrics["base_devices"] == 161
    assert metrics["deterministic_openocd_exact_icpns"] == 468
    assert metrics["ic_support_bound_exact_icpns"] == 2
    assert metrics["unresolved_programming_profile_exact_icpns"] == 466
