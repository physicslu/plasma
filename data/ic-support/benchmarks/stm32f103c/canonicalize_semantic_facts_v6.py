...
import canonicalize_semantic_facts_v5 as relationship_base
import semantic_extraction_v6 as semantic

HERE = Path(__file__).resolve().parent
DEFAULT_CONTRACT = HERE / "canonicalization-contract-v4.json"
...
def _dedupe_citations(citations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for citation in citations:
        ...
        require(isinstance(source_id, str), "applicability citation source_id required")
        require(
            isinstance(page_index, int) and not isinstance(page_index, bool),
            "applicability citation physical_page_index required",
        )
...
def _authority_units(authority: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    require(isinstance(catalogs, dict), "applicability authority catalogs required")
...
def validate_programming_applicability_projection(applicability: dict[str, Any]) -> None:
    require(
        applicability.get("source_lock_id") == "stm32f103c-source-lock-v0",
        "programming applicability source-lock mismatch",
    )
    ...
    require(isinstance(authority_file, str) and authority_file, "applicability authority file required")
    ...
    require(isinstance(projected_targets, dict), "programming applicability targets required")
...
def derive_programming_relationship(
    applicability: dict[str, Any],
    contract: dict[str, Any],
    *,
    verify_authority: bool = True,
) -> tuple[str, list[dict[str, Any]], dict[str, Any]]:
    relationship_contract = contract.get("relationship_derivation", {}).get("programming")
    require(isinstance(relationship_contract, dict), "programming relationship derivation contract required")
    require(
        relationship_contract.get("source") == "evidence_backed_applicability",
        "programming relationship must derive from applicability",
    )
    if verify_authority:
        validate_programming_applicability_projection(applicability)

    targets = applicability.get("targets")
    require(isinstance(targets, dict), "programming applicability targets object required")
    ...
    if any(normalized[target] is None for target in TARGETS):
        ...
        require(relationship == "unknown", "incomplete programming applicability must resolve to unknown")
        citations = []
    else:
        ...
        require(relationship in {"shared", "different"}, "invalid derived programming relationship")

    transformation = {
        "kind": "DETERMINISTIC_APPLICABILITY_RELATIONSHIP_DERIVATION",
        "path": "$.canonical_spec.profile_relationships.programming",
        ...
        "input": target_inputs,
        "comparison": {"semantics": "applicable_contract_identity"},
        "output": relationship,
    }
...
def _adapt_for_relationship_base(
    ...
    require(isinstance(applicability_name, str) and applicability_name, "programming applicability file required")
    applicability = load_json(HERE / applicability_name)
    programming_relationship, relationship_citations, transformation = derive_programming_relationship(
        applicability,
        contract,
    )
    ...
    adapted_facts = {
        **facts,
        "profile_relationships": {
            "programming": programming_relationship,
            "option": relationships["option"],
            "security": relationships["security"],
        },
    }
    adapted_evidence = dict(evidence)
    if programming_relationship != "unknown":
        adapted_evidence["$.semantic_facts.profile_relationships.programming"] = relationship_citations
...
def canonicalize_response(payload: dict[str, Any], contract: dict[str, Any]) -> dict[str, Any]:
    require(
        contract.get("semantic_schema_id") == "plasma://ic-support/stm32f103c/semantic-extraction-v4",
        "unexpected programming relationship semantic schema ID",
    )
    require(
    ...
    require(base_contract_name == "canonicalization-contract-v3.json", "unexpected relationship base contract")
    base_contract = load_json(HERE / base_contract_name)

    adapted_payload, programming_transformation = _adapt_for_relationship_base(payload, contract)
    ...
    report["transformations"] = [programming_transformation, *report["transformations"]]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Derive programming/memory/package relationships and canonicalize semantic facts")
    ...
    try:
        ...
        print("IC applicability/relationship-derived canonicalization COMPLETE")
        ...
        print("- programming relationship emitted by AI: false")
        ...
        print(f"IC applicability/relationship-derived canonicalization FAIL: {exc}", file=sys.stderr)
        return 1
