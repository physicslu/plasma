...
SEMANTIC_SCHEMA = HERE / "semantic-extraction-v7.schema.json"
PROMPT_TEMPLATE = HERE / "semantic-extraction-prompt-v7.txt"
SEMANTIC_EXPERIMENT_ID = "stm32f103c-semantic-extraction-v7"
SEMANTIC_RUN_SCHEMA_VERSION = "0.8.0"
...
def render_prompt(context: str) -> tuple[str, dict[str, Any]]:
    ...
    return rendered, {
        ...
        "identity_ontology": {
            "target_map_key": "requested_commercial_icpn",
            "manufacturer_applicability_expression": "evidence_backed_manufacturer_scope_expression",
        },
    }
...
def validate_semantic_facts(facts: dict[str, Any]) -> None:
    ...
    errors = shared.validate_against_schema(facts, schema, root_schema=schema, path="$.semantic_facts")
    require(not errors, "semantic schema validation failed: " + "; ".join(errors[:10]))


def parse_model_result(raw_text: str, *, allowed_pages: dict[str, set[int]] | None = None) -> dict[str, Any]:
    text = raw_text.strip()
