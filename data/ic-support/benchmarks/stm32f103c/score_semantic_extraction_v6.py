...
GROUND_TRUTH = HERE / "semantic-extraction-ground-truth-v4.json"
SCORE_SCHEMA_VERSION = "0.5.0"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Score one STM32F103C v6 semantic extraction run")
    ...
    try:
        ...
        report = {
            ...
            "score_authority": {"ground_truth_file": GROUND_TRUTH.name, "ground_truth_sha256": harness.sha256_bytes(GROUND_TRUTH.read_bytes()), "generation_reads_score_authority": False},
            "score": score,
        }
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        print("IC v6 semantic extraction scoring COMPLETE")
        ...
        print(f"IC v6 semantic extraction scoring FAIL: {exc}", file=sys.stderr)
        return 1
