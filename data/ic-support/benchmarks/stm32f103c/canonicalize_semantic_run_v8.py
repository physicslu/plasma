...
PIPELINE_SCHEMA_VERSION = "0.7.0"
PIPELINE_ID = "stm32f103c-all-profile-relationships-derived-semantic-to-canonical-v6"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Canonicalize one retained STM32F103C v8 semantic extraction run")
    ...
    try:
        ...
        print("IC v8 semantic run canonicalization COMPLETE")
        print(f"- status: {result['canonicalization_status']}")
        print(f"- security relationship: {result['canonical_spec']['profile_relationships']['security']}")
        print(f"- option relationship: {result['canonical_spec']['profile_relationships']['option']}")
        print(f"- programming relationship: {result['canonical_spec']['profile_relationships']['programming']}")
        ...
        print(f"IC v8 semantic run canonicalization FAIL: {exc}", file=sys.stderr)
        return 1
