...
import score_semantic_extraction_v8 as shared
import semantic_extraction_v9 as semantic

HERE = Path(__file__).resolve().parent
GROUND_TRUTH = HERE / "semantic-extraction-ground-truth-v7.json"
SCORE_SCHEMA_VERSION = "0.8.0"

SemanticScoreError = shared.SemanticScoreError
require = shared.require
load_json = shared.load_json
is_missing = shared.is_missing
...
def score_run(run: dict[str, Any]) -> dict[str, Any]:
    ...
    for path, expected_value in expected_leaves.items():
        ...
        if is_missing(actual):
            ...
            continue
        if shared._equivalent(path=path, actual=actual, expected=expected_value, comparison_rules=comparison_rules):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Score one STM32F103C v9 semantic extraction run")
    ...
    try:
        ...
        print("IC v9 semantic extraction scoring COMPLETE")
        ...
        print(f"IC v9 semantic extraction scoring FAIL: {exc}", file=sys.stderr)
        return 1
