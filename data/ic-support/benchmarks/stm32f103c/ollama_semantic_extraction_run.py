...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run one STM32F103C manufacturer-near semantic extraction through bounded Ollama native chat"
    )
    ...
    try:
        ...
        print(
            f"Ollama semantic extraction {record['status']}: arm={args.arm}; "
        ...
        print(f"Ollama semantic extraction FAIL: {exc}", file=sys.stderr)
        return 1
