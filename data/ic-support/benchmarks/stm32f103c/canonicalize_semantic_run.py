...
PIPELINE_ID = "stm32f103c-semantic-to-canonical-v0"
...
def canonicalize_run(run_path: Path, contract_path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    ...
    # Re-validate the retained generation artifact before crossing the deterministic boundary.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Canonicalize one retained STM32F103C semantic extraction run")
    ...
    try:
        ...
        print("IC semantic run canonicalization COMPLETE")
        ...
        print(f"IC semantic run canonicalization FAIL: {exc}", file=sys.stderr)
        return 1
