    ...
    *,
    ...
    record: dict[str, Any] = {
        ...
        "trust_boundary": {
            ...
            "structured_option_semantics": True,
    ...
    try:
        ...
        parsed = semantic.parse_model_result(raw_text, allowed_pages=allowed_pages)
    ...
    stem = f"{arm_name}.semantic-v3"
    (output_dir / f"{stem}.raw.txt").write_text(raw_text, encoding="utf-8")
    (output_dir / f"{stem}.run.json").write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run one STM32F103C structured manufacturer-near semantic extraction through bounded Ollama native chat"
    )
    ...
    try:
        ...
        print(
            f"Ollama structured semantic extraction {record['status']}: arm={args.arm}; "
        ...
        print(f"Ollama structured semantic extraction FAIL: {exc}", file=sys.stderr)
        return 1
