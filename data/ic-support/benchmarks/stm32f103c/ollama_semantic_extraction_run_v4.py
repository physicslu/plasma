...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run one STM32F103C per-target package-hardware semantic extraction through bounded Ollama native chat"
    )
    ...
    try:
        ...
        print(
            f"Ollama relationship-derivation semantic extraction {record['status']}: arm={args.arm}; "
        ...
        print(f"Ollama relationship-derivation semantic extraction FAIL: {exc}", file=sys.stderr)
        return 1
