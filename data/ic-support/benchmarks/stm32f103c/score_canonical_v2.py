...
import canonicalize_semantic_run as pipeline
import semantic_extraction_v2 as semantic

HERE = Path(__file__).resolve().parent
GROUND_TRUTH = HERE / "canonical-ground-truth-v0.json"
...
def score_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    require(
        canonicalization.get("canonicalization_id") == truth.get("canonicalization_id"),
    ...
    uncited_count, out_of_context_count, out_of_context_paths = _validate_canonical_evidence(
        actual,
        evidence,
        context=context,
    )
    ...
    return {
        ...
        "paths": {
            "wrong": sorted(wrong),
            "missing_unknown": sorted(missing),
            "out_of_context_citation": out_of_context_paths,
        },
        "trust_boundary": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Score one canonicalized STM32F103C v2 semantic extraction run")
    ...
    try:
        ...
        print("IC canonical v2 scoring COMPLETE")
        ...
        print(f"- transformations: {score.get('transformation_count')}")
        ...
        print(f"IC canonical v2 scoring FAIL: {exc}", file=sys.stderr)
        return 1
