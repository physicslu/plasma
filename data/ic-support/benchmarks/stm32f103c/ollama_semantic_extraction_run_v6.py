...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one STM32F103C v6 semantic extraction through bounded Ollama native chat")
    ...
    try:
        ...
        print(f"Ollama v6 semantic extraction {record['status']}: arm={args.arm}; input_tokens={record.get('usage', {}).get('input_tokens')}; generation_tokens={record.get('usage', {}).get('generation_tokens')}")
        ...
        print(f"Ollama v6 semantic extraction FAIL: {exc}", file=sys.stderr)
        return 1
