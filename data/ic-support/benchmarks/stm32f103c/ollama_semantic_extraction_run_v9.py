...
def execute_arm(*, workspace: Path, arm_name: str, output_dir: Path, ollama_url: str, model: str,
    ...
    record: dict[str, Any] = {
        ...
        "prompt": prompt_meta,
        "runtime": {
            "transport": "ollama_native_chat", "runtime_label": runtime_label,
            "model_id": model, "ollama_url": ollama_url,
    ...
    try:
        response = ollama_transport.ollama_native_chat(
            ollama_url=ollama_url, model=model, prompt=prompt, num_ctx=num_ctx,
        ...
        parsed = semantic.parse_model_result(
            raw_text,
            allowed_pages=semantic.allowed_pages_from_run_context(run_context)
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one STM32F103C v9 semantic extraction through bounded Ollama native chat")
    ...
    try:
        record = execute_arm(
            workspace=args.workspace, arm_name=args.arm, output_dir=args.output_dir,
        ...
        print(f"Ollama v9 semantic extraction {record['status']}: arm={args.arm}; input_tokens={record.get('usage', {}).get('input_tokens')}; generation_tokens={record.get('usage', {}).get('generation_tokens')}")
        ...
        print(f"Ollama v9 semantic extraction FAIL: {exc}", file=sys.stderr)
        return 1
