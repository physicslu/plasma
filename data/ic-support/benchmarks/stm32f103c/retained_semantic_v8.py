...
import ollama_semantic_extraction_run_v8 as ollama_runner
...
BUNDLE_ID = "stm32f103c-qwen-v8-retained-model-proof-v0"
FILES = {
    "raw": "reduced_context.semantic-v8.raw.txt",
    "run": "reduced_context.semantic-v8.run.json",
...
def verify_bundle(output_dir: Path) -> dict[str, Any]:
    ...
    require(run.get("experiment_id") == semantic.SEMANTIC_EXPERIMENT_ID, "v8 experiment ID mismatch")
    ...
    relationships = facts.get("profile_relationships")
    require(relationships == {}, "v8 AI output must contain no profile relationships")
    ...
    for key in (
        "wrong_semantic_assertion_count",
        "missing_unknown_count",
        "uncited_assertion_count",
        "out_of_context_citation_count",
    ):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Execute or verify the STM32F103C Qwen v8 retained model proof")
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--verify-only", action="store_true")
    parser.add_argument("--workspace", type=Path)
    parser.add_argument("--ollama-url", default="http://127.0.0.1:11434")
    parser.add_argument("--runtime-label", default="mac-m3max-48gb-ollama-qwen3.8-27b-mlx-64k-semantic-v8")
    ...
    try:
        ...
        print("STM32F103C v8 retained model proof VERIFIED")
        ...
        print("- AI profile relationships: false")
        ...
        print(f"STM32F103C v8 retained model proof FAIL: {exc}", file=sys.stderr)
        return 1
