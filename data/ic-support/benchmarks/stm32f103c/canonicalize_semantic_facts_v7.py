...
import canonicalize_semantic_facts_v6 as relationship_base
import semantic_extraction_v7 as semantic

HERE = Path(__file__).resolve().parent
DEFAULT_CONTRACT = HERE / "canonicalization-contract-v5.json"
...
def _dedupe_citations(citations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for citation in citations:
        ...
        require(isinstance(source_id, str), "option applicability citation source_id required")
        require(
            isinstance(page_index, int) and not isinstance(page_index, bool),
            "option applicability citation physical_page_index required",
        )
...
def _authority_units(authority: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    require(isinstance(catalogs, dict), "option applicability authority catalogs required")
...
def validate_option_applicability_projection(applicability: dict[str, Any]) -> None:
    ...
    require(isinstance(authority_file, str) and authority_file, "option applicability authority file required")
    ...
    require(
        ...
        "option applicability target set mismatch",
    )
    declared_contract = applicability.get("option_contract_id")
    require(isinstance(declared_contract, str) and declared_contract, "option contract id required")
    ...
    for target, projected in projected_targets.items():
        ...
        require(contract_id == declared_contract, f"{target}: projected option contract mismatch")
...
def derive_option_relationship(
    applicability: dict[str, Any],
    contract: dict[str, Any],
    *,
    verify_authority: bool = True,
) -> tuple[str, list[dict[str, Any]], dict[str, Any]]:
    relationship_contract = contract.get("relationship_derivation", {}).get("option")
    require(isinstance(relationship_contract, dict), "option relationship derivation contract required")
    require(
        relationship_contract.get("source") == "evidence_backed_applicability",
        "option relationship must derive from applicability",
    )
    require(
        relationship_contract.get("scope") == "benchmark_profile_projection_only",
        "option relationship derivation scope mismatch",
    )
    if verify_authority:
        validate_option_applicability_projection(applicability)

    targets = applicability.get("targets")
    require(isinstance(targets, dict), "option applicability targets object required")
    ...
    if any(normalized[target] is None for target in TARGETS):
        ...
        require(relationship == "unknown", "incomplete option applicability must resolve to unknown")
        citations = []
    else:
        ...
        require(relationship in {"shared", "different"}, "invalid derived option relationship")

    transformation = {
        "kind": "DETERMINISTIC_APPLICABILITY_RELATIONSHIP_DERIVATION",
        "path": "$.canonical_spec.profile_relationships.option",
        "relationship": "option",
        ...
        "input": target_inputs,
        "comparison": {
            "semantics": "applicable_option_contract_identity",
...
def _adapt_for_relationship_base(
    ...
    require(isinstance(applicability_name, str) and applicability_name, "option applicability file required")
    applicability = load_json(HERE / applicability_name)
    option_relationship, relationship_citations, transformation = derive_option_relationship(
        applicability,
        contract,
    )
    ...
    adapted_facts = {
        **facts,
        "profile_relationships": {
            "option": option_relationship,
            "security": relationships["security"],
        },
    }
    adapted_evidence = dict(evidence)
    if option_relationship != "unknown":
        adapted_evidence["$.semantic_facts.profile_relationships.option"] = relationship_citations
...
def canonicalize_response(payload: dict[str, Any], contract: dict[str, Any]) -> dict[str, Any]:
    require(
        contract.get("semantic_schema_id") == "plasma://ic-support/stm32f103c/semantic-extraction-v5",
        "unexpected option relationship semantic schema ID",
    )
    require(
    ...
    require(base_contract_name == "canonicalization-contract-v4.json", "unexpected relationship base contract")
    base_contract = load_json(HERE / base_contract_name)

    adapted_payload, option_transformation = _adapt_for_relationship_base(payload, contract)
    ...
    report["transformations"] = [option_transformation, *report["transformations"]]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Derive option/programming/memory/package relationships and canonicalize semantic facts")
    ...
    try:
        ...
        print("IC option/applicability relationship-derived canonicalization COMPLETE")
        ...
        print("- option relationship emitted by AI: false")
        ...
        print(f"IC option/applicability relationship-derived canonicalization FAIL: {exc}", file=sys.stderr)
        return 1
