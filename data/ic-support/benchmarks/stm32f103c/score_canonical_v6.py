...
GROUND_TRUTH = HERE / "canonical-ground-truth-v4.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Score one canonicalized STM32F103C v6 semantic extraction run")
    ...
    try:
        ...
        output = {
            ...
            "score_authority": {"ground_truth_file": GROUND_TRUTH.name, "ground_truth_sha256": harness.sha256_bytes(GROUND_TRUTH.read_bytes()), "generation_reads_ground_truth": False},
            "score": score,
        }
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(json.dumps(output, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        print("IC canonical v6 scoring COMPLETE")
        ...
        print(f"IC canonical v6 scoring FAIL: {exc}", file=sys.stderr)
        return 1
