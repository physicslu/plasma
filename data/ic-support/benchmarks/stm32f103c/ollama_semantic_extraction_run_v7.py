...
def execute_arm(*, workspace: Path, arm_name: str, output_dir: Path, ollama_url: str, model: str,
    ...
    record: dict[str, Any] = {
        ...
        "trust_boundary": {
            ...
            "option_applicability_visible_to_generation": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one STM32F103C v7 semantic extraction through bounded Ollama native chat")
    ...
    try:
        ...
        print(f"Ollama v7 semantic extraction {record['status']}: arm={args.arm}; input_tokens={record.get('usage', {}).get('input_tokens')}; generation_tokens={record.get('usage', {}).get('generation_tokens')}")
        ...
        print(f"Ollama v7 semantic extraction FAIL: {exc}", file=sys.stderr)
        return 1
